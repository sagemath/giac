...
Il est conseill\'e de lire ce document apr\`es avoir lanc\'e {\tt Xcas} (sous Windows,
cliquer sur le raccourci {\tt xcasfr.bat}, sous linux Gnome, cliquer sur Xcas dans
le menu Education ou en tapant depuis un Terminal {\tt xcas \&} puis la touche {\tt enter},
sur Mac en cliquant sur xcas dans le menu Applications), 
...
\index{menus}
\item[$\bullet$]
un onglet indiquant le nom de la session, ou {\tt Unnamed} si la session n'a pas
encore été sauvegardée,
\item[$\bullet$]
une zone de gestion de la session avec un bouton \verb|?| pour ouvrir l'index de l'aide
un bouton \verb@Save@ pour sauvegarder la session, 
un bouton \verb@Config: exact real ...@ affichant la configuration et permettant de la modifier,
...
\item[$\bullet$]
une zone rectangulaire blanche num\'erot\'ee 1 (première ligne
de commande) dans laquelle vous pouvez taper
votre premi\`ere commande (cliquez si nécessaire pour faire
...
\begin{itemize}
\item soit l'index de l'aide s'ouvre à la commande sélectionnée (par
exemple pour les commandes du menu \verb|CAS|). Cliquez sur le bouton
\verb|Details| pour afficher la page du manuel correspondant à la commande
dans votre navigateur.
\item soit une boite de dialogue s'ouvre vous permettant de spécifier
les arguments de la commande (par exemple pour tracer une courbe
depuis le menu \verb|Graphic|)
\item soit la commande est recopiée dans la ligne de commande. Pour connaitre
la syntaxe de cette commande, appuyez sur le bouton \verb|?| en haut à gauche,
...
Si vous connaissez déjà le nom d'une commande et que vous d\'esirez
...
(située \`a gauche de la touche A sur un clavier
fran\c{c}ais) ou cliquer sur le bouton \verb|?| en haut à gauche. 
L'index des commandes appara\^\i t alors dans une fen\^etre, positionné 
à la première compl\'etion possible, avec une aide succinte sur chaque
commande. 
...
les r\'eponses. Nous insistons sur le fait que pour {\tt Xcas}, 
\verb@ax@ est une
...
\begin{itemize}
\item[$\bullet$]
\verb@2*a+b@ retourne $2a+b$
\item[$\bullet$]
\verb@a/2*b@ retourne $\displaystyle \frac{ab}{2}$
\item[$\bullet$]
\verb@a/2/b@ retourne $\displaystyle \frac{a}{b2}$
\item[$\bullet$]
\verb@a^2*b@ retourne $a^2b$
\end{itemize}
...
garde num\'ero qu'elle avait. On peut rappeler par \verb@ans()@ (answer) la 
...
d\'efinit la fonction $f: x \mapsto E$ car \verb@E@ n'est pas évalué.
Par contre \verb@E:= x^2-1; f:=unapply(E,x)@ d\'efinit bien la
fonction $f: x\mapsto x^2-1$ car \verb@E@ est évalué.
...
d'it\'eration comme \verb@$@ ou \verb@seq@ (qui it\`erent une
...
\hline\hline
\verb@diff(ex)@ &d\'eriv\'ee d'une expression\\
\verb@function_diff(f)@ & d\'eriv\'ee d'une fonction\\
...
Pour les obtenir, il faut activer l'option \verb@Complex@ \`a
partir du bouton rouge sur fond gris \verb@cas@ (en bas \`a gauche). 
...
Les fonctions de traitement des polyn\^omes figurent dans le menu 
\verb@Alg->Polynomes@. 
...
faut activer l'option \verb@Complex@ \`a partir du bouton rouge
\verb@cas@. On peut aussi d\'eclarer les coefficients comme des
entiers modulo $p$ pour travailler dans $\Z/p\Z$ (commande \verb@%@) 
ou dans un corps
fini (défini par la commande \verb@GF@). Ex\'ecutez les
...
Le menu \verb|Math->Transcendental| contient 
...
\begin{itemize}
\item[$\bullet$] 
\verb|Math->Réecriture_trig|~: transformation des tangentes en sinus et
cosinus ({\tt tan2sincos}), transformations en $\tan(x/2)$ ({\tt halftan}),\ldots
\item[$\bullet$] 
\verb|Math->Réecriture_trig_exp|~: transformation des fonctions 
trigonom\'etriques en exponentielles par les
formules d'Euler ({\tt trig2exp}), des exponentielles en fonctions trigonom\'etriques ({\tt exp2trig}), des exponentielles en puissances ({\tt exp2pow})...,
\item[$\bullet$] 
\verb|Math->Réecriture_trig_inv|~: transformation des fonctions inverses
\end{itemize}
\begin{verbatim}
exp2pow(exp(3*ln(x)))
exp2trig(exp(i*x))
...
vous devez séparer les commandes les créant par un \verb|;|. Vous pouvez aussi créer
...
ordonn\'ee) peuvent \^etre chang\'es dans la fenêtre de configuration
graphique accessible depuis le menu
\verb@Cfg@, item \verb@Configuration graphique@.
\index{ClrGraph@{\verb+ClrGraph+}}
\index{effacer@effacer}

Notez enfin que les objets graphiques 2-d sont 
aussi affich\'es dans une fen\^etre appelée
\verb@DispG@ (Display Graphics) que vous pouvez faire appara\^\i tre par le menu
...
il est conseillé d'utiliser le menu \verb|Graphic|.

L'instruction de tracé d'une courbe représentative de fonction est \verb|plot| avec en
...
\verb|color=| suivi de la liste des couleurs \`a utiliser. Les
...
\hline\hline
\verb@plot@ & graphe d'une expression\\
\verb@color@ & param\`etre de \verb@plot@ pour la couleur\\
...
\verb@point@ & point donn\'e par son affixe ou 2 coordonnées\\
\verb@segment@ & segment donnée par 2 points\\
\verb@droite@ & droite donnée par son équation ou 2 points\\
\verb@cercle@ & cercle donnée par centre et rayon\\
\verb@inter@ & intersection de courbes\\
\verb@equation@ & équation cartésienne\\
\verb@parameq@ & équation paramétrique\\
...
%
\begin{itemize}
\item Pour tracer une surface d\'efinie par l'équation
...
fen\^etre de visualisation (la fen\^etre de calcul par défaut est 
d\'efinie par la configuration graphique si on ne l'a pas indiquée
en param\`etre \verb|plotfunc|)
...
\end{verbatim}
\item On peut aussi tracer des objets géométriques 3D dans une figure 3-d que l'on 
...
\hline\hline
\verb@plotfunc@ & surface par équation \\
\verb@plotparam@ & surface ou courbe paramétrique\\
\hline\hline
\verb@point@ & point donn\'e par 3 coordonnées\\
\verb@droite@ & droite donnée par 2 équations ou 2 points\\
\verb|plan| & plan donné par 1 équation ou 3 points \\
\verb@sphere@ & sphère donnée par centre et rayon\\
\verb@cone@ & cone donné par centre, axe, angle d'ouverture \\
...
\end{itemize}
\index{table}
Dans un programme, lorsqu'on appelle une variable munie d'un indice qui n'est pas affectée
à une liste, séquence ou matrice, 
...
Si \verb@a@ est une variable formelle, la commande \verb@a[1]:=2@
cr\'ee une table \verb|a|.
Pour que \verb|a| soit une liste, il faut d'abord affecter \verb|a|
avec une liste de taille suffisante, par exemple \verb|a:=[0$10]|
%$
(si on conna\^\i t la taille de la liste) ou 
\verb|a:=[]; a:=append(a,3)|. 
...
\end{verbatim}
(le \verb@else@ est facultatif). La condition est un bool\'een,
...
\index{for@{\verb+for+}}
\index{boucle}
La syntaxe de \verb@for@ est la suivante.
\begin{verbatim}
...
un \'editeur de programme avec le menu \verb@Add->Programme@. Le menu
\verb@Prog@ de l'\'editeur permet d'entrer facilement les structures
...
\end{verbatim}
Saisissez cette fonction dans un \'editeur de programme, testez-la
...
Le débuggueur affiche automatiquement la valeur des paramètres \verb|a,b| puis
des variables locales \verb|q,r| lors de l'exécution instruction par instruction avec
le bouton \verb|sst|.
%
...
pour la première, en éliminant la première avec \verb|quo| et en
trouvant les deux dernières solutions en résolvant l'équation
du second degré (utiliser \verb|coeff| pour calculer le discriminant
de l'équation).
\item
