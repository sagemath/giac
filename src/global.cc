/* -*- compile-command: "g++-3.4 -I.. -g -c global.cc" -*- */
#include "first.h"
/*  
...
namespace giac {
  ...
  gen (*fl_widget_unarchive_function)(istream &,const vecteur & v)=0;
  ...
  double _epsilon_=100*numeric_limits<double>::epsilon();
#else
  double _epsilon_=1e-10;
#endif
  ...
  bool _complex_variables_=false; 
  ...
  bool _angle_radian_=true;
  bool & angle_radian(GIAC_CONTEXT){
    if (contextptr && contextptr->globalptr )
      return contextptr->globalptr->_angle_radian_;
    else
      return _angle_radian_;
  }

  void angle_radian(bool b,GIAC_CONTEXT){
    if (contextptr && contextptr->globalptr )
      contextptr->globalptr->_angle_radian_=b;
    else
      _angle_radian_=b;
  }

  bool _show_point_=true;
  ...
  bool _variables_are_files_=false;
  ...
  thread_param _thread_param;
  thread_param * _thread_param_ptr=&_thread_param;

  thread_param * thread_param_ptr(const context * contextptr){
    return (contextptr && contextptr->globalptr)?contextptr->globalptr->_thread_param_ptr:_thread_param_ptr;
  }

  bool kill_thread(GIAC_CONTEXT){
    thread_param * ptr= (contextptr && contextptr->globalptr )?contextptr->globalptr->_thread_param_ptr:0;
    return ptr?ptr->_kill_thread:_thread_param_ptr->_kill_thread;
  }

  void kill_thread(bool b,GIAC_CONTEXT){
    thread_param * ptr= (contextptr && contextptr->globalptr )?contextptr->globalptr->_thread_param_ptr:0;
    if (!ptr)
      ptr=_thread_param_ptr;
    ptr->_kill_thread=b;
  }
  ...
  int thread_eval_status(GIAC_CONTEXT){
    ...
    if (contextptr && contextptr->globalptr){
      ...
      res=_thread_param_ptr->thread_eval_status;
  ...
  void thread_eval_status(int val,GIAC_CONTEXT){
    if (contextptr && contextptr->globalptr){
      ...
      pthread_mutex_lock(&_mutex_eval_status);
      _thread_param_ptr->thread_eval_status=val;
      pthread_mutex_unlock(&_mutex_eval_status);
    }
  }
  ...
  int _eval_level=DEFAULT_EVAL_LEVEL;
  ...
  unsigned int _rand_seed=123457;
...
#else
#ifdef GNUWINCE
    srand(rand_seed(contextptr));
    int res=rand();
    rand_seed(res,contextptr);
    return res;
#endif
#ifdef WIN32
    return random();
  ...
  void error_token_name(const std::string & b,GIAC_CONTEXT){
    if (contextptr && contextptr->globalptr )
  ...
  std::vector<logo_turtle> _turtle_stack_(1,_turtle_);
  ...
  string xcasroot; // used by WIN32 for the path to the xcas directory
#ifdef WIN32
...
#endif
#ifndef HAVE_NO_SIGNAL_H
    cerr << getpid() << "Ctrl-C pressed" << endl;
#endif
  }
  ...
  */
  ...
  gen wait_parent(){
    ...
    gen res;
    try {
      res=unarchive(child_in,list_one_letter__IDNT,context0);
    }
  ...
  vecteur subvect(const vecteur & v,int i){
    int s=v.size();
    if (i<0)
      i=-i;
    vecteur res(v);
  ...
  pid_t make_child(){ // forks and return child id
	...
	// Unarchive step
	try {
	  child_in >> rpn_mode >> global_window_ymin >> history_begin_level ;
	  // cerr << args << endl;
	  if (history_begin_level<0){
	    child_in >> synchronize_history;
	    args=unarchive(child_in,list_one_letter__IDNT,context0);
  ...
  void archive_write_error(){
    cerr << "Archive error on " << cas_entree_name() << endl;
  }
  
  // return true if entree has been sent to evalation by child process
  bool child_eval(const string & entree,bool numeric,bool is_run_file){
#if defined(HAVE_NO_SIGNAL_H) || defined(DONT_FORK)
    return false;
#else
    if (is_run_file || rpn_mode)
	...
	parent_out << rpn_mode << " " << global_window_ymin << " " << history_begin_level << endl;
	...
	entr = gen(s,list_one_letter__IDNT,context0);
	if (entr.type!=_VECT)
	...
	entr = gen(entree,list_one_letter__IDNT,context0);
	if (numeric)
  ...
  bool child_reeval(int history_begin_level){
    ...
    try {
      ofstream parent_out(cas_entree_name().c_str());
      parent_out << rpn_mode << " " << global_window_ymin << " " << -1-history_begin_level << " " << synchronize_history << endl;
      if (synchronize_history){
  ...
  unary_function_ptr parent_evalonly_sommets[]={at_widget_size,at_keyboard,at_current_sheet,at_Row,at_Col,0};
  bool update_data(gen & entree,gen & sortie,GIAC_CONTEXT){
	...
	if (entree.type==_FUNC){
	  int s=min(max(entree.subtype,0),history_out(contextptr).size());
	  vecteur v(s);
	  for (int k=s-1;k>=0;--k){
  ...
  bool read_data(gen & entree,gen & sortie,string & message,GIAC_CONTEXT){
    if (!data_ready)
      return false;
    message="";
    try {
	...
	setsizeerr();
      clock_t start, end;  
      start = clock();
      entree=unarchive(parent_in,list_one_letter__IDNT,contextptr);
      sortie=unarchive(parent_in,list_one_letter__IDNT,contextptr);
      end = clock();
  ...
  void read_config(const string & name,GIAC_CONTEXT,bool verbose){
    ...
    if (!inf)
      return;
    vecteur args,l(list_one_letter__IDNT);
    if (verbose)
      cerr << "// Reading config file " << name << endl;
    readargs_from_stream(inf,args,l,contextptr);
    gen g(args);
  ...
  string geo_setup_string(){
    return xyztrange(gnuplot_xmin,gnuplot_xmax,gnuplot_ymin,gnuplot_ymax,gnuplot_zmin,gnuplot_zmax,gnuplot_tmin,gnuplot_tmax,global_window_xmin,global_window_xmax,global_window_ymin,global_window_ymax,_show_axes_,class_minimum,class_size,gnuplot_hidden3d+2*gnuplot_pm3d).print(context0);
  }
  ...
  vector<context *> context_list(1,(context *) 0);
  ...
  void init_context(context * ptr){
    if (ptr){
      ...
      ptr->globalptr->_angle_radian_=_angle_radian_;
  ...
  global::global() : _decimal_digits_(12), _scientific_format_(0), _xcas_mode_(0), _integer_format_(0), _latex_format_(0),_expand_re_im_(true), _do_lnabs_(true), _eval_abs_(true),_complex_mode_(false), _complex_variables_(false), _increasing_power_(false), _approx_mode_(false), _angle_radian_(true), _variables_are_files_(false), _bounded_function_no_(0), _series_flags_(0x3),_local_eval_(true), _default_color_(0), _epsilon_(1e-10), _proba_epsilon_(1e-15), _withsqrt_(true), _show_point_(true),  _io_graph_(true), _show_axes_(1),_spread_Row_ (-1), _spread_Col_ (-1), _all_trig_sol_(false), _ntl_on_(true),_logptr_(&std::cerr), _prog_eval_level_val(1), _eval_level(DEFAULT_EVAL_LEVEL), _rand_seed(123457),_max_sum_sqrt_(3),_max_sum_add_(100000),_total_time_(0),_evaled_table_(0) { 
  ...
  global & global::operator = (const global & g){
     ...
     _angle_radian_=g._angle_radian_;
