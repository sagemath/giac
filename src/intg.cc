// -*- mode:C++ ; compile-command: "g++-3.4 -I.. -g -c intg.cc" -*-
#include "first.h"
/*
...
namespace giac {
  ...
  bool has_nop_var(const vecteur & v){
  ...
  gen nop_inv(const gen & e){
    return symbolic(at_nop,gen(symbolic(at_inv,e)));
  }
  gen nop_pow(const gen & e){
    ...
    if ( (e._VECTptr->back().type!=_INT_) || (e._VECTptr->back().val>=0) || ( (e._VECTptr->front().type==_SYMB) && (e._VECTptr->front()._SYMBptr->sommet==at_exp) ) )
      return symbolic(at_pow,e);
    return nop_inv(symbolic(at_pow,makevecteur(e._VECTptr->front(),-e._VECTptr->back())));
  }

  gen sin_over_cos(const gen & e){
    return rdiv(symb_sin(e),symb_cos(e));
  }
  vecteur lvarxwithinv(const gen &e,const identificateur & x,GIAC_CONTEXT){
    vector<unary_function_ptr> vu(1,at_inv);
    vu.push_back(at_pow);
    vu.push_back(at_tan);
    vector< gen (*) (const gen &) > ve(1,nop_inv);
    ve.push_back(nop_pow);
    ve.push_back(sin_over_cos);
    gen ee=subst(e,vu,ve,false,contextptr);
  ...
  bool is_linear_wrt(const gen & e,const identificateur &x,gen & a,gen & b,GIAC_CONTEXT){
    ...
    if (!is_constant_wrt(a,x,contextptr))
      return false;
    b=ratnormal(e-a*x);
    return true;
  }
  ...
  bool is_quadratic_wrt(const gen & e,const identificateur &x,gen & a,gen & b,gen & c,GIAC_CONTEXT){
    ...
    if (!is_linear_wrt(tmp,x,a,b,contextptr))
      return false;
  ...
  // eval N at e=x*exp(i*dephasage*pi/n)/(X-e)+conj and integrate
  gen substconj(const gen & N,const gen & X,const gen & x,gen dephasage,GIAC_CONTEXT){
    gen c=cos(dephasage,contextptr);
    gen s=sin(dephasage,contextptr);
    gen e=x*(c+cst_i*s);
    gen b=normal(subst(N,X,e,false,contextptr),contextptr);
    gen res=re(b,contextptr)*ln(pow(X,2)+normal(-2*c*x,contextptr)*X+x.squarenorm(contextptr),contextptr); 
    gen N2=normal(-2*im(b,contextptr),contextptr); // same
    gen atanterm=atan((X-c*x)/(s*x),contextptr);
    if (X.is_symb_of_sommet(at_tan))
      atanterm += cst_pi*sign(s*x,contextptr)*_floor((X._SYMBptr->feuille/cst_pi+plus_one_half),contextptr);
    res=res+N2*atanterm;
    return res;
  }
  ...
  gen _surd(const gen & args,GIAC_CONTEXT){
    if (args.type!=_VECT || args._VECTptr->size()!=2 || args._VECTptr->back().type!=_INT_)
      setsizeerr();
    return surd(args._VECTptr->front(),args._VECTptr->back().val,contextptr);
  }
  const string _surd_s("surd");
  unary_function_eval __surd(&_surd,_surd_s);
  unary_function_ptr at_surd (&__surd,0,true);

  gen makelnatan(const gen & N,const gen & X,const gen & c0,int n,GIAC_CONTEXT){
    gen c(c0),res(0);
    if (n%2){
	...
	c=pow(c,inv(n,contextptr),contextptr);
      ...
      for (int i=1;i<=n/2;++i)
	res += substconj(N,X,c,2*i*cst_pi/n,contextptr);
      return true;
    }
    if (is_positive(c,contextptr) ){
      c=pow(c,inv(n,contextptr),contextptr);
      res += normal(subst(N,X,c,false,contextptr),contextptr)*lnabs(X-c,contextptr)+normal(subst(N,X,-c,false,contextptr),contextptr)*lnabs(X+c,contextptr);
      for (int i=1;i<n/2;++i)
	res += substconj(N,X,c,2*i*cst_pi/n,contextptr);
    }
    else {
      c=pow(-c,inv(n,contextptr),contextptr);
      for (int i=0;i<n/2;++i)
	res += substconj(N,X,c,(2*i+1)*cst_pi/n,contextptr);
    }
    return res;
  }

  // integration of cyclotomic-type denominators
  bool integrate_deno_length_2(const polynome & num,const vecteur & v,const vecteur & l,const vecteur & lprime,gen & res,GIAC_CONTEXT){
    if (v.size()<2)
      return false;
	...
	if (is_zero(im(a,contextptr)) && is_zero(im(b,contextptr)) && is_zero(im(c,contextptr))){
	  if (is_positive(delta,contextptr)){
	    res += makelnatan(N/c2,X,c2,d,contextptr);
	    res -= makelnatan(N/c1,X,c1,d,contextptr);
	    return true;
	  }
	  else {
	    ...
	    for (int i=0;i<d;++i)
	      res += substconj(N/c2,X,moduled,(argument+2*i*cst_pi)/d,contextptr);
	  }
	  return true;
	}
    ...
    gen c=-b/a;
    if (d!=n){ // rescale and check cyclotomic
      gen tw=r2e(v/v.front(),lprime,contextptr);
      if (tw.type!=_VECT)
	return false;
	...
	*jt=normal(*jt * pow(c,-k),contextptr);
	...
	for (int i=1;i<=deg/2;++i){
	  if (gcd(i,deg)!=1)
	    continue;
	  res += substconj(N,X,c,2*i*cst_pi/deg,contextptr);
	}
      }
      return true;
    }
	...
	return true;
      ...
      res += makelnatan(N,X,c,n,contextptr);
      return true;
    }
  }
  ...
  int is_symmetric(const vecteur & v,vecteur & res,bool sym){
    if (v.empty())
      return 0;
    int n=v.size();
    vecteur w;
  ...
  void xtoinvx(const gen & a,const modpoly & n,const modpoly & d,modpoly & newn, modpoly & newd,bool dx){
  ...
  gen integrate_rational(const gen & e, const identificateur & x, gen & remains_to_integrate,gen & xvar,GIAC_CONTEXT){
    ...
    if (num_itend-num_it>1){
      precedent=num_it->index.iptr->front(); 
      ++num_it;
      for (;num_it!=num_itend;++num_it){
	actuel=num_it->index.iptr->front();
	...
	if (is_symmetric(q,N,true)*is_symmetric(r,D,true)==1){
      ...
      if (!type && DivRem(D,test,0,q,r) && r.empty()){
	r=N;
    ...
    vector< pf<gen> > intde_VECT,rest,finalde_VECT;
    for (;it!=itend;++it){
      pf<gen> single(intreduce_pf(*it,intde_VECT));
      // factor(single.den,p_content,vden,false,withsqrt(contextptr),complex_mode(contextptr));
      factor(single.den,p_content,vden,false,false,false);
    ...
    vecteur lprime(l);
    if (lprime.front().type!=_VECT) setsizeerr("in integrate_rational");
    ...
    for (;it!=itend;++it){
      ...
      switch (deg) {
      case 1: // 1st order
	lnpart=lnpart+rdiv(r2e(it->num,l,contextptr),r2e(firstcoeff(it->den),l,contextptr))*lnabs(r2e(it->fact,l,contextptr),contextptr);
	break; 
      case 2: // 2nd order
	findabcdelta(it->fact,a,b,c,delta);
	uselog=is_positive(r2e(delta,lprime,contextptr),contextptr);
	...
	lnpart=lnpart+rdiv(r2e(d,lprime,contextptr),r2e(lnpartden,lprime,contextptr))*lnabs(r2e(it->fact,l,contextptr),contextptr);
	...
	if (uselog){
	  ...
	  lnpart=lnpart+rdiv(r2e(atannum,lprime,contextptr),(r2e(alpha,lprime,contextptr))*sqrtdelta)*lnabs(rdiv(deuxaxplusb-sqrtdelta,deuxaxplusb+sqrtdelta),contextptr);
	}
	else {
	  sqrtdelta=normal(sqrt(r2e(-delta,lprime,contextptr),contextptr),contextptr);
	  ...
	  if (tmpatan.is_symb_of_sommet(at_tan))
	    ...
	    if (xvar.is_symb_of_sommet(at_tan)){
	      ...
	      residue=ratnormal(limit(tmpatan,x,0,-1,contextptr)-limit(tmpatan,x,0,1,contextptr));
	...
	if (!c.coord.empty()){
	  ...
	  if (!integrate_deno_length_2(c,vtmp,l,lprime,lnpart,contextptr))
	    rest.push_back(pf<gen>(c,a*it->den,it->fact,1));
	}
    ...
    remains_to_integrate=r2sym(rest,l,contextptr);
    return rdiv(r2e(ipnum.integrate(),l,contextptr),r2e(ipden,l,contextptr))+r2sym(intde_VECT,l,contextptr)+lnpart;
  }

  // integration of e when linear operations have been applied
  gen xln_x(const gen & x,GIAC_CONTEXT){
    return x*ln(x,contextptr)-x;
  }

  gen int_exp(const gen & x,GIAC_CONTEXT){
    return exp(x,contextptr);
  }

  gen int_sinh(const gen & x,GIAC_CONTEXT){
    return cosh(x,contextptr);
  }

  gen int_cosh(const gen & x,GIAC_CONTEXT){
    return sinh(x,contextptr);
  }

  gen int_sin(const gen & x,GIAC_CONTEXT){
    if (angle_radian(contextptr))
      return -cos(x,contextptr);
    else
      return -cos(x,contextptr)*rad2deg_e;
  }

  gen int_cos(const gen & x,GIAC_CONTEXT){
    if (angle_radian(contextptr))
      return sin(x,contextptr);
    else
      return sin(x,contextptr)*rad2deg_e;
  }

  gen int_tan(const gen & x,GIAC_CONTEXT){
  ...
  gen int_tanh(const gen & x,GIAC_CONTEXT){
    return -ln(cosh(x,contextptr),contextptr);
  }

  gen int_asin(const gen & x,GIAC_CONTEXT){
    if (angle_radian(contextptr))
  ...
  gen int_acos(const gen & x,GIAC_CONTEXT){
    if (angle_radian(contextptr))
  ...
  gen int_atan(const gen & x,GIAC_CONTEXT){
    if (angle_radian(contextptr)) 
  ...
  gen_op_context primitive_tab_primitive[]={giac::int_sin,giac::int_cos,giac::int_tan,giac::int_exp,giac::int_sinh,giac::int_cosh,giac::int_tanh,giac::int_asin,giac::int_acos,giac::int_atan,giac::xln_x};

  void solve_aPprime_plus_P(const gen & anum,const gen & aden,const vecteur & Q,vecteur & R,gen & Pden){
    ...
    for (int k=n-1;it!=itend;++it,--k){
      R.push_back(*it-rdiv(gen(k+1),anum)*R.back()*aden);
    }
    // should simplify R with Pden
  }

  void insure_real_deno(gen & n,gen & d,GIAC_CONTEXT){
  ...
  bool is_rewritable_as_f_of(const gen & fu,const gen & u,gen & fx,const identificateur & x,GIAC_CONTEXT){
    gen a,b;
    if (is_linear_wrt(u,x,a,b,contextptr)){
      fx=complex_subst(fu,x,rdiv(x-b,a),contextptr);
      return false;// true?
    }
	...
	if (!is_rewritable_as_f_of(fu,tmpu,tmpfx,x,contextptr))
	  return false;
	fx=complex_subst(tmpfx,x,-x,contextptr);
	return true;
      }
	...
	if (tmpu.type==_VECT && tmpu._VECTptr->size()==2){
	  ...
	  if (expo.type==_INT_){ 
	    if (is_linear_wrt(tmpu,x,a,b,contextptr)){
	      fx=complex_subst(fu,x,rdiv(pow(x,expo,contextptr)-b,a),contextptr);
	      return true;
	    }
	  }
	}
	...
	if (u._SYMBptr->feuille.type!=_VECT)
	  return is_rewritable_as_f_of(fu,u._SYMBptr->feuille,fx,x,contextptr);
	decompose_prod(*u._SYMBptr->feuille._VECTptr,x,non_constant,alpha,contextptr);
	if (non_constant.empty()) setsizeerr("in is_rewritable_as_f_of_f");
	if (!is_one(alpha)){
	  gen tmpu,tmpfx;
	  tmpu=_prod(non_constant,contextptr);
	  if (!is_rewritable_as_f_of(fu,tmpu,tmpfx,x,contextptr))
	    return false;
	  fx=complex_subst(tmpfx,x,rdiv(x,alpha),contextptr);
	  return true;
	}
      }
	...
	if (u._SYMBptr->feuille.type!=_VECT)
	  return is_rewritable_as_f_of(fu,u._SYMBptr->feuille,fx,x,contextptr);
	decompose_plus(*u._SYMBptr->feuille._VECTptr,x,non_constant,alpha,contextptr);
	if (non_constant.empty()) setsizeerr("in is_rewritable_as_f_of_f 2");
	if (!is_zero(alpha)){
	  gen tmpu,tmpfx;
	  tmpu=_plus(non_constant,contextptr);
	  if (!is_rewritable_as_f_of(fu,tmpu,tmpfx,x,contextptr))
	    return false;
	  fx=complex_subst(tmpfx,x,x-alpha,contextptr);
	  return true;
	}
      }
    }
    return in_is_rewritable_as_f_of(fu,u,fx,x,contextptr);
  }
  ...
  bool in_is_rewritable_as_f_of(const gen & fu,const gen & u,gen & fx,const identificateur & x,GIAC_CONTEXT){
    if (fu.type==_VECT){
      ...
      gen tmp;
      for (;it!=itend;++it){
	if (!is_rewritable_as_f_of(*it,u,tmp,x,contextptr))
	  return false;
	res.push_back(tmp);
      }
      fx=res;
    ...
    if (in_is_rewritable_as_f_of(f,u,tmpfx,x,contextptr)){
      fx=symbolic(s,tmpfx);
      return true;
    }
	...
	fx=pow(1-pow(x,2),fexp);
	return true;
      case 3: // sin^2=1-1/(tan^2+1)
	fx=pow(1-inv(pow(x,2)+1,contextptr),fexp);
	return true;
      default:
	return false;
      }
	...
	fx=pow(inv(1-pow(x,-2),contextptr)-1,fexp);
	return true;
  ...
  vecteur lvarxpow(const gen &e,const identificateur & x){
    ...
    // do lvar again to _VECTrim the first arg of ^
    return lvar(res);
  }
  ...
  gen integrate(const gen & e_orig,const gen & x_orig,gen & remains_to_integrate,GIAC_CONTEXT){
    if (x_orig.type!=_IDNT){
      identificateur x(" x");
      gen e=subst(e,x_orig,x,false,contextptr);
      e=integrate(e,x,remains_to_integrate,contextptr);
      remains_to_integrate=quotesubst(remains_to_integrate,x,x_orig,contextptr);
      return quotesubst(e,x,x_orig,contextptr);
    }
    const identificateur & x =*x_orig._IDNTptr;
    return integrate(e_orig,x,remains_to_integrate,contextptr);
  }

  gen integrate(const gen & e_orig,const identificateur & x,gen & remains_to_integrate,GIAC_CONTEXT){
    gen e(e_orig);
	...
	if (piecef.type!=_VECT){
	  ...
	  return integrate(e,x,remains_to_integrate,contextptr);
	}
	...
	for (int i=0;i<nargs/2;++i){
	  remainsv[2*i+1]=0;
	  piecev[2*i+1]=integrate(piecev[2*i+1],x,remainsv[2*i+1],contextptr);
	  addremains = addremains || !is_zero(remainsv[2*i+1]);
	}
	if (nargs%2){
	  remainsv[nargs-1]=0;
	  piecev[nargs-1]=integrate(piecev[nargs-1],x,remainsv[nargs-1],contextptr);
	...
	if (tmp.type!=_VECT || tmp._VECTptr->size()!=3)
	  setsizeerr("Bad when "+it->print(contextptr));
	...
	l4.push_back(*it);
      ...
      *logptr(contextptr) << "Warning, integration of abs or sign assumes constant sign by intervals (correct if the argument is real):\nCheck " << l1 << endl;
      ...
      gen res=integrate(e,x,remains_to_integrate,contextptr),resadd;
      // check what happens when si==0
      for (int j=0;j<i;++j){
	gen val=l4[j],a,b,r;
	if (val.is_symb_of_sommet(at_sign)){
	  if (is_linear_wrt(val._SYMBptr->feuille,x,a,b,contextptr)){
	    r=-b/a;
	    vecteur l5(l4);
	    l5[j]=1;
	    ...
	    gen tmp=ratnormal((limit(liminf,x,r,-1,contextptr)-limit(limsup,x,r,1,contextptr))/2)*val;
      ...
      return resadd+complex_subst(res,l3,l4,contextptr);
    }
    // Step1: detection of some unary_op[linear fcn]
    unary_function_ptr u=e._SYMBptr->sommet;
    gen f=e._SYMBptr->feuille,a,b;
    // particular case for ^, _FUNCnd arg must be constant
    if ( (u==at_pow) && is_constant_wrt(f._VECTptr->back(),x,contextptr) && is_linear_wrt(f._VECTptr->front(),x,a,b,contextptr) ){
      b=f._VECTptr->back();
      if (is_minus_one(b))
    ...
    if (s && is_linear_wrt(f,x,a,b,contextptr) )
    ...
    if (rvarsize<TRY_FU_UPRIME){ // otherwise no hope
      ...
      ++it; // don't try x!
      for (;it!=itend;++it){
	gen df=derive(*it,x,contextptr);
	fu=ratnormal(rdiv(e,df));
	if (is_undef(fu) && is_zero(ratnormal(df))){
	  // *it is constant -> find the value
	  gen newvar=subst(*it,x,zero,false,contextptr);
	  gen newe=subst(e,*it,newvar,false,contextptr);
	  return integrate(newe,x,remains_to_integrate,contextptr);
	}
	if (is_rewritable_as_f_of(fu,*it,fx,x,contextptr)){
	  gen tmprem;
	  gen tmpres=linear_integrate(fx,x,tmprem,contextptr);
	  remains_to_integrate=remains_to_integrate+complex_subst(tmprem,x,*it,contextptr)*df;
	  return complex_subst(tmpres,x,*it,contextptr);
	}
	if (it->type!=_SYMB)
	  continue;
	...
	if (f.type!=_SYMB)
	  continue;
	if (is_linear_wrt(f,x,a,b,contextptr))
	  continue;
	df=derive(f,x,contextptr);
	fu=ratnormal(rdiv(e,df));
	if (is_rewritable_as_f_of(fu,f,fx,x,contextptr)){
	  gen tmprem;
	  gen tmpres=linear_integrate(fx,x,tmprem,contextptr);
	  remains_to_integrate=remains_to_integrate+complex_subst(tmprem,x,f,contextptr)*df;
	  return complex_subst(tmpres,x,f,contextptr);
	}	  
      }
    }
    ...
    for (int i=0;(i<TRY_FU_UPRIME) && (vt!=vtend);++vt,++i){
      gen tmprem,u=linear_integrate(*vt,x,tmprem,contextptr);
      if (!is_zero(tmprem))
	continue;
      if (!est_puissance){
	vecteur vv(v);
	...
	fu=symbolic(at_prod,vv);
      ...
      if (is_rewritable_as_f_of(fu,u,fx,x,contextptr)){
	gen tmpres=linear_integrate(fx,x,tmprem,contextptr);
	remains_to_integrate=remains_to_integrate+complex_subst(tmprem,x,u,contextptr)*derive(u,x,contextptr);
	return complex_subst(tmpres,x,u,contextptr);
      }
    }
    ...
    if (rvarsize==1){
      gen xvar(x);
      return integrate_rational(e,x,remains_to_integrate,xvar,contextptr);
    }

    // square roots
    if ( (rvarsize==2) && (rvar.back().type==_SYMB) && (rvar.back()._SYMBptr->sommet==at_pow) ){ // x and a power
	...
	if (is_linear_wrt(argument,x,a,b,contextptr)){
	  // argument=(ax+b)=t^d -> x=(t^d-a)/b and dx=d/a*t^(d-1)*dt
	  vecteur substin(makevecteur(argument,x));
	  vecteur substout(makevecteur(pow(x,d),rdiv(pow(x,d)-b,a)));
	  tmpe=complex_subst(e,substin,substout,contextptr)*pow(x,d-1);
	  tmpres=linear_integrate(tmpe,x,tmprem,contextptr);
	  gen fnc_inverse=pow(a*x+b,fraction(1,d),contextptr);
	  remains_to_integrate=rdiv(d,a)*complex_subst(tmprem,x,fnc_inverse,contextptr);
	  return rdiv(d,a)*complex_subst(tmpres,x,fnc_inverse,contextptr);
	}
	vecteur tmpv(1,x);
	lvar(argument,tmpv);
	...
	if (is_linear_wrt(fr_n,x,a,b,contextptr) && is_linear_wrt(fr_d,x,ap,bp,contextptr) ){
	  // argument=(a*x+b)/(ap*x+bp)=t^d
	  // -> x=(bp*t^d-b)/(a-ap*t^d) 
	  // -> dx= d*(b*ap-a*bp)*t^(d-1)/(a-ap*t^d)^2
	  vecteur substin(makevecteur(argument,x));
	  vecteur substout(makevecteur(pow(x,d),rdiv(bp*pow(x,d)-b,a-ap*pow(x,d))));
	  tmpe=complex_subst(e,substin,substout,contextptr)*rdiv(pow(x,d-1),pow(a-ap*pow(x,d),2));
	  tmpres=linear_integrate(tmpe,x,tmprem,contextptr);
	  gen fnc_inverse=pow(rdiv(a*x+b,ap*x+bp),fraction(1,d),contextptr);
	  gen tmp=gen(d)*(a*bp-b*ap);
	  remains_to_integrate=tmp*complex_subst(tmprem,x,fnc_inverse,contextptr);
	  return tmp*complex_subst(tmpres,x,fnc_inverse,contextptr);
	}
	...
	*/
	if (d==2 && is_constant_wrt(fr_d,x,contextptr)){
	  ...
	  den=C[2];
	  gen alpha,beta,xvar(x);
	  if (!is_linear_wrt(num,tmpx,beta,alpha,contextptr))
	    setsizeerr();
	  alpha=integrate_rational(alpha/den,x,remains_to_integrate,xvar,contextptr);
	  ...
	  */
	  ...
	  C=_quorem(makevecteur(D,y,x),contextptr);
	  int l=0;
	  if (is_zero(C[1])){ // P/(den/y)
	    ...
	    P=P*y;
	  gen yl=pow(y,l);
	  C=_egcd(makevecteur(D,yl,x),contextptr);
	  gen g=C[2],Q=C[0],R=C[1];
	  C=_quorem(makevecteur(P*R,D,x),contextptr);
	  gen PD=C[1]/g;
	  gen Py=(P*Q+C[0]*yl)/g;
	  C=_quorem(makevecteur(Py,y,x),contextptr);
	  /*
	   ...
	   */
	  gen tmpv=_e2r(makevecteur(Py,x),contextptr);
	  if (tmpv.type!=_VECT) setsizeerr();
	  vecteur colP=*tmpv._VECTptr;
	  int n=colP.size()-1;
	  tmpv=_e2r(makevecteur(y,x),contextptr);
	  if (tmpv.type!=_VECT) setsizeerr();
	  ...
	  if (n){
	    lrdm(colP,n);
	    gen yprime=(1-2*l)*derive(y,x,contextptr);
	    matrice sys;
	    tmpv=_e2r(makevecteur(yprime,x),contextptr);
	    if (tmpv.type!=_VECT) setsizeerr();
	    ...
	    sys.push_back(col);
	    col0.push_back(zero);
	    tmpv=_e2r(makevecteur(2*y,x),contextptr);
	    if (tmpv.type!=_VECT) setsizeerr();
	    ...
	    tmpv=_e2r(makevecteur(2*yl,x),contextptr);
	    if (tmpv.type!=_VECT) setsizeerr();
	    col0=*tmpv._VECTptr;
	    for (int i=0;i<k-1;++i){
	      col=col0;
	    ...
	    C=2*horner(vecteur(col0.begin(),col0.begin()+k-1),x);
	    Q=2*horner(vecteur(col0.begin()+k-1,col0.end()),x);
	    alpha=alpha+Q*sqrt(y,contextptr)/yl;
	  }
	  else
	    C=Py;
	  e=(C+PD/D)/sqrt(argument,contextptr);
	  if (is_quadratic_wrt(argument,x,a,b,c,contextptr)){
	    if (!is_positive(-a,contextptr)){
	      gen sqrta(sqrt(a,contextptr));
	      identificateur id_m(" m");
	      gen m(id_m);
	      tmpe=eval(rdiv(complex_subst(e*sqrt(argument,contextptr),argument,pow(m+sqrta*x,2),contextptr),b-plus_two*sqrta*m),1,contextptr);
	      tmpe=complex_subst(tmpe,x,rdiv(m*m-c,b-plus_two*sqrta*m),contextptr);
	      tmpres=linear_integrate(tmpe,id_m,tmprem,contextptr);
	      remains_to_integrate=remains_to_integrate+complex_subst(plus_two*tmprem,m,sqrt(argument,contextptr)-sqrta*x,contextptr);
	      return alpha+complex_subst(plus_two*tmpres,m,sqrt(argument,contextptr)-sqrta*x,contextptr);
	    }
	    else {
		...
		( *			dx/sqrt[y]=2*du/[sqrt[-a]*[1+u^2]] * )
	      */
	      ...
	      gen u(id_u),uu(u);
	      gen uasx=rdiv(plus_two*sqrta*sqrt(argument,contextptr)-D,plus_two*a*x+b);
	      gen sqrty=rdiv(D-(plus_two*a*x+b)*u,plus_two*sqrta);
	      tmpe=eval(rdiv(complex_subst(e*sqrt(argument,contextptr),argument,pow(sqrty,2),contextptr),1+u*u),1,contextptr);
	      tmpe=complex_subst(tmpe,x,rdiv(rdiv(plus_two*u*D,1+u*u)-b,plus_two*a),contextptr);
	      tmpres=integrate_rational(tmpe,id_u,tmprem,uu,contextptr);
	      // sqrt(a*x^2+b*x+c) -> a*[(x+b/2/a)^2-(D/a)^2]
	      // -> asin(a*x+b/2)
	      vecteur vin(makevecteur(u,symbolic(at_atan,u))),vout(makevecteur(uasx,inv(2,contextptr)*asin(ratnormal((-2*a*x-b)/abs(D,contextptr)),contextptr)));
	      remains_to_integrate=remains_to_integrate+complex_subst(rdiv(plus_two,sqrta)*tmprem,vin,vout,contextptr);
	      return alpha+complex_subst(rdiv(plus_two,sqrta)*tmpres,vin,vout,contextptr);
	    }
	...
	xt=rdiv(symbolic(inverse_sommet,x)-b,a);
	dxt=derive(xt,x,contextptr);
	vecteur substin(makevecteur(x,*rvt));
	vecteur substout(makevecteur(xt,x));
	tmpe=complex_subst(e,substin,substout,contextptr)*dxt;
	tmpres=linear_integrate(tmpe,x,tmprem,contextptr);
	remains_to_integrate=complex_subst(rdiv(tmprem,dxt),x,*rvt,contextptr);
	tmpres=_texpand(tmpres,contextptr);
	return complex_subst(tmpres,substout,substin,contextptr);
      }
    }
	...
	vecteur algx;
	if (lvarx(ibpe,x)==vecteur(1,x)){
	  gen tmpres,tmprem,tmpprimitive,tmp,xvar(x);
	  tmpprimitive=integrate_rational(ibpe,x,tmp,xvar,contextptr);
	  if (is_zero(tmp) && lvarx(tmpprimitive,x)==vecteur(1,x)){
	    tmpres=tmpprimitive*derive(*ibp,x,contextptr);
	    tmpres=recursive_normal(tmpres,true,contextptr);
	    tmpres=linear_integrate(tmpres,x,tmprem,contextptr);
	...
	test=equalposcomp(inverse_tab_op,e._SYMBptr->sommet);
      if (test){
	gen tmpres,tmprem;
	tmpres=linear_integrate(x*derive(e,x,contextptr),x,tmprem,contextptr);
	remains_to_integrate=-tmprem;
	return x*e-tmpres;
      }
    }
    ...
    // rewrite inv(exp)
    vector<unary_function_ptr> vsubstin(1,at_inv);
	...
	++it;
	if (!is_quadratic_wrt(reaxb,x,rex2,rea,reb,contextptr)){
	...
	if (!is_zero(rex2)){
	  if (is_zero(im(rex2,contextptr)) && is_positive(-rex2,contextptr)){
	    vecteur vx2(lvarxpow(coeff,x));
	    if ( vx2.size()>1 || (!vx2.empty() && vx2.front()!=x) ){
	      remains_to_integrate = remains_to_integrate + coeff*exp(reaxb,contextptr);
	      continue;
	    }
	    ...
	    coeff=quotesubst(coeff,x,x-decal,contextptr);
	    // exp(cst)*subst(int(exp(rex2*x^2)*coeff(x),x),x,x+decal)
	    vecteur les_var(1,x); // insure x is the main var
	    ...
	    gen ppart=subst(r2e(poly12polynome(vres,1,les_vars),les_var,contextptr),x,x+decal,false,contextptr)/r2e(vresden,vecteur(les_var.begin()+1,les_var.end()),contextptr)*exp(reaxb,contextptr);
	    // add erf part from the last coeff vres[vs]
	    gen a=-rex2; // r2e(-ina,les_var,contextptr);
	    gen sqrta=sqrt_noabs(a,contextptr);
	    gen erfpart=r2e(in_coeffnumv[vs],cdr_VECT(les_var),contextptr)*symbolic(at_sqrt,cst_pi)/sqrta*exp(cst,contextptr)/2*_erf(sqrta*(x+decal),contextptr);
	...
	for (;jt!=jtend;){
	  ...
	  // check polynomial
	  // FIXME: use Ei/Si/Ci -> lvarx in vx2
	  vecteur vx2(lvarxpow(coeff,x));
	  if ( (vx2.size()>1) || ( (!vx2.empty()) && (vx2.front()!=x))  || ( (imaxb.type==_SYMB) && !is_linear_wrt(imaxb._SYMBptr->feuille,x,ima,imb,contextptr) ) ){
	  ...
	  if (trig_type)
	    ...
	    if (is_zero(rea)){
	      gen tmprem,xvar(x);
	      res= res + exp(reb,contextptr)*integrate_rational(coeff,x,tmprem,xvar,contextptr);
	  ...
	  if (trig_type){
	    ...
	    rewrite_with_t_real(imcoeff,x,contextptr);
	  ...
	  if (coeff_is_real){
	    ...
	    rewrite_with_t_real(resre,x,contextptr);
 	    gen resim=im(resplus,contextptr);
	    rewrite_with_t_real(resim,x,contextptr);
	    if (trig_type==1)
      ...
      return res+risch(tmp,x,remains_to_integrate,contextptr);
    } // end linearizable
	...
	gen a,b;
	if (!is_linear_wrt(vart->_SYMBptr->feuille,x,a,b,contextptr)){
	  trig_fraction=false;
	  continue;
	}
    ...
    if (trig_fraction){
      ...
      vecteur substout;
      gen a,b,coeff_cst;
      is_linear_wrt(vart->_SYMBptr->feuille,x,a,b,contextptr);
	...
	if (n.type!=_INT_) setsizeerr("trig_fraction");
	gen angle=ratnormal(b-n*coeff_cst);
	substout.push_back(symbolic(vart->_SYMBptr->sommet,n*x+angle));
      }
      ...
      if (trig_fraction==4){ // everything depends on exp(x)
          f=complex_subst(f,exp(x,contextptr),x,contextptr)*inv(x,contextptr);
          tmpres=linear_integrate(f,x,tmprem,contextptr);
          gen expx=exp(coeff_trig*x+coeff_cst,contextptr);
          remains_to_integrate = inv(coeff_trig,contextptr)*complex_subst(tmprem,x,expx,contextptr);
          return inv(coeff_trig,contextptr)*complex_subst(tmpres,x,expx,contextptr);
      }
      f=halftan(f,contextptr); // now everything depends on tan(x/2)
      // t=tan(x/2), dt=1/2(1+t^2)*dx
      gen xsur2=rdiv(coeff_trig*x+coeff_cst,plus_two);
      gen tanxsur2=tan(xsur2,contextptr);
      f=complex_subst(f,tan(rdiv(x,plus_two),contextptr),x,contextptr)*inv(plus_one+pow(x,2),contextptr);
      vecteur vf(1,x);
      rlvarx(f,x,vf);
      if (vf.size()<=1)
	tmpres=integrate_rational(f,x,tmprem,tanxsur2,contextptr);
      else {
	tmpres=linear_integrate(f,x,tmprem,contextptr);
    ...
    /*
    // detect here polynomial denominator wrt x, check for Ei/Si/Ci
    var=vecteur(1,x);
    lvar(e,var);
    fraction nd=e2r(e,var,contextptr);
    gen num,den;
    fxnd(nd,num,den);
    vecteur vx2=lvarx(r2sym(den,var,contextptr),x);
    if (vx2.size()==1 && vx2.front()==x){
      // Reduction of denominator by square free partial fraction
      // Then try integrate_ei_si_ci here
      cerr << den << endl;
    }
    */
    // finish by calling the Risch algorithm
    return risch(e,x,remains_to_integrate,contextptr);
  }
  ...
  gen linear_integrate(const gen & e,const identificateur & x,gen & remains_to_integrate,GIAC_CONTEXT){
    ...
    return linear_apply(ee,x,remains_to_integrate,contextptr,integrate);
  }
  ...
  gen integrate(const gen & e,const identificateur & x,GIAC_CONTEXT){
    if (e.type==_VECT){
      ...
      for (;it!=itend;++it)
	w.push_back(integrate(*it,x,contextptr));
      return w;
    }
    ...
    if (is_zero(remains_to_integrate))
      ...
      return res+symbolic(at_integrate,makevecteur(remains_to_integrate,x));
  }

  gen integrate0(const gen & e,const identificateur & x,gen & remains_to_integrate,GIAC_CONTEXT){
    if (e.type==_VECT){
      vecteur w;
      vecteur::const_iterator it=e._VECTptr->begin(),itend=e._VECTptr->end();
      for (;it!=itend;++it)
  ...
  gen integrate(const gen & e,const gen & f,GIAC_CONTEXT){
    if (f.type!=_IDNT){
      ...
      return quotesubst(integrate(e1,x,contextptr),x,f,contextptr);
    }
    return integrate(e,*f._IDNTptr,contextptr);
  }
  ...
  gen _integrate(const gen & args,GIAC_CONTEXT){
    ...
    if ( (s==3) || (s==1) )
      toofewargs("integrate");
    if (s>4)
      toomanyargs("integrate");
    gen x=v[1];
    if (x.type!=_IDNT){
      identificateur t(" t");
    ...
    bool quoted=*x._IDNTptr->quoted;
    *x._IDNTptr->quoted=true;
    v[0]=eval(v[0],eval_level(contextptr),contextptr);
    for (int i=2;i<s;++i){
      v[i]=eval(v[i],eval_level(contextptr),contextptr);
    }
    *x._IDNTptr->quoted=quoted;    
    if (approx_mode(contextptr) && (s==4))
      return _romberg(makevecteur(v[0],x,v[2],v[3]),contextptr);
    ...
    if (s==4){
      ...
      lfloor=lvarx(lfloor,*x._IDNTptr);
      if (!lfloor.empty()){
	gen a,b,l,cond=lfloor.front()._SYMBptr->feuille,tmp;
	if (!is_linear_wrt(cond,*x._IDNTptr,a,b,contextptr))
	    setsizeerr("Floor definite integration: can only handle linear < or > condition");
	...
	return res;
      ...
      lpiece=lvarx(lpiece,*x._IDNTptr);
      if (!lpiece.empty()){
	gen piece=lpiece.front();
	if (!piece.is_symb_of_sommet(at_piecewise))
	  setsizeerr();
	...
	for (int i=0;i<vs/2;++i){
	  ...
	  gen a,b,l;
	  if (unable || !is_linear_wrt(cond,*x._IDNTptr,a,b,contextptr))
	    setsizeerr("Piecewise definite integration: can only handle linear < or > condition");
    ...
    if (s==2){
      if (is_zero(rem))
	return primitive;
      return primitive + symbolic(at_integrate,makevecteur(rem,x));
    }
    // here s==4
    ...
    vecteur sp=find_singularities(primitive,*x._IDNTptr,false,contextptr);
    ...
    if (!is_zero(rem)){
      if (is_inf(res))
	return symbolic(at_integrate,makevecteur(v[0],x,v[2],v[3]));
      res = res + symbolic(at_integrate,makevecteur(rem,x,v[2],v[3]));
    }
    return res;
  }
  const string _integrate_s("integrate");
  string texprintasintegrate(const gen & g,const string & s_orig,GIAC_CONTEXT){
    string s("\\int ");
    if (g.type!=_VECT)
  ...
  unary_function_eval __integrate(&_integrate,_integrate_s,0,&texprintasintegrate);
  unary_function_ptr at_integrate (&__integrate,_QUOTE_ARGUMENTS,true);
  ...
  gen romberg(const gen & f0,const gen & x0,const gen & a,const gen &b,const gen & eps,GIAC_CONTEXT,int nmax=10){
    gen x(x0),f(f0);
    if (x.type!=_IDNT){
      x=identificateur(" x");
    ...
    if (is_inf(old_line[0])|| is_undef(old_line[0])){
      ...
      *logptr(contextptr) << "Infinity or undefined limit at bounds.\nUsing middle point Romberg method with double precision" << endl;
    ...
    for (int i=0;i<nmax;++i){
      ...
      gen y=a+h/2,sum;
      for (int j=0;j<n;++j){
	sum=sum+evalf(subst(f,x,y,false,contextptr),eval_level(contextptr),contextptr);
	y=y+h;
      }
      h=h/2;
	...
	cur_line.push_back((pui4*cur_line[j]-old_line[j])/(pui4-1));
      ...
      if (i>nmax/2 && ck_is_greater(eps,abs(old_line[i]-cur_line[i+1],contextptr),contextptr))
  ...
  gen _romberg(const gen & args,GIAC_CONTEXT) {
    ...
    if (eps.type!=_DOUBLE_ || eps.type!=_REAL)
    ...
    if ( x.type!=_IDNT || 
	 ...
	 || (b.type!=_DOUBLE_ && b.type!=_REAL) 
      ...
      return symbolic(at_integrate,args);
    return romberg(f,x,a,b,eps,contextptr,n);
  }
  const string _romberg_s("romberg");
  unary_function_eval __romberg(&_romberg,_romberg_s);
  unary_function_ptr at_romberg (&__romberg,0,true);
...
*		0   0	0	n+1     X^n
**********************************************************************/
  vecteur solveP_x_plus_1_minus_P_x(const vecteur & Q){
  ...
  bool is_shift_of(const vecteur & v1,const vecteur &v2,int & n){
  ...
  bool rational_sum(const gen & e,const gen & x,gen & res,gen& remains_to_sum,bool allow_psi,GIAC_CONTEXT){
    ...
    vecteur v(lvarxpow(e,*x._IDNTptr));
    if (v.empty()){
      res = e*x;
      remains_to_sum = 0;
      return true;
    ...
    factorization vden;
    factor(den,p_content,vden,false,/* withsqrt */false,/* complex */ true); 
    vector< pf<gen> > pfde_VECT;
    ...
    vector<pf1> shiftfree_pfde_VECT;
    vector< pf<gen> >::iterator it=pfde_VECT.begin();
    vector< pf<gen> >::const_iterator itend=pfde_VECT.end();
    for (;it!=itend;++it){
      vecteur it_fact(polynome2poly1(it->fact,1));
      vector<pf1>::iterator jt=shiftfree_pfde_VECT.begin();
      vector<pf1>::const_iterator jtend=shiftfree_pfde_VECT.end();
      int k;
      for (;jt!=jtend;++jt){
	if (is_shift_of(it_fact,jt->fact,k))
      ...
      if (jt==jtend)
	shiftfree_pfde_VECT.push_back(pf1(it->num,it->den,it->fact,it->mult));
  ...
  void AB2PQR(const polynome & A,const polynome & B,polynome & P,polynome & Q, polynome & R){
    ...
    index_t i=transposition(0,1,s+1);
    a.reorder(i);
    b.reorder(i);
  ...
  bool gosper(const polynome & P,const polynome & Q,const polynome & R,polynome & Y,gen & deno,GIAC_CONTEXT){
    ...
    if (q==r && qq==rr){ // cancellation
      ...
      y=p-max(q,r);
      if (q>0){
	vecteur vq=polynome2poly1(Q),vr=polynome2poly1(R);
  ...
  gen inner_sum(const gen & e,const identificateur & x,gen & remains_to_sum,GIAC_CONTEXT){
    gen res;
    if (rational_sum(e,x,res,remains_to_sum,true,contextptr))
      return res;
    polynome P,Q,R;
    vecteur v;
    if (!is_hypergeometric(e,x,v,P,Q,R,contextptr)){
      remains_to_sum=e;
      return zero;
    }
    int s=v.size();
  ...
  gen sum(const gen & e,const gen & x,gen & remains_to_sum,GIAC_CONTEXT){
    if (x.type!=_IDNT)
      setsizeerr();
    vecteur v=lvarx(e,*x._IDNTptr);
    v=lop(v,sincostan_v);
    if (!v.empty()){
      gen w=trig2exp(v,contextptr);
      gen e1=_lin(subst(e,v,*w._VECTptr,true,contextptr),contextptr);
      return _simplify(_evalc(linear_apply(e1,*x._IDNTptr,remains_to_sum,contextptr,inner_sum),contextptr),contextptr); 
    }
    else
      return linear_apply(e,*x._IDNTptr,remains_to_sum,contextptr,inner_sum); 
  }
  ...
  gen sum(const gen & e,const gen & x,const gen & a,const gen &b,GIAC_CONTEXT){
    ...
    gen res;
    if ( (is_inf(a) || is_inf(b) ) && sumab(e,x,a,b,res,contextptr) )
      return res;
    gen remains_to_sum;
  ...
  void local_sto(const gen & value,const identificateur & i,GIAC_CONTEXT){
    if (contextptr)
      (*contextptr->tabptr)[*i.name]=value;
    else
  ...
  void local_sto_int(int value,const identificateur & i,GIAC_CONTEXT){
    if (contextptr)
      (*contextptr->tabptr)[*i.name].val=value;
    else
      i.localvalue->back().val=value;
  }

  void local_sto_int_increment(int value,const identificateur & i,GIAC_CONTEXT){
    if (contextptr)
      (*contextptr->tabptr)[*i.name].val += value;
    else
      i.localvalue->back().val += value;
  ...
  gen seqprod(const gen & g,int type,GIAC_CONTEXT){
	...
	step=-step;
      for (;is_greater(debut,fin,contextptr);debut=debut+step){
	tmp=quotesubst(v[0],v[1],debut,contextptr);
	res.push_back(quotesubst(eval(tmp,contextptr),v[1],debut,contextptr));
      }
    }
    else {
      if (is_positive(-step,contextptr))
	step=-step;
      for (;is_greater(fin,debut,contextptr);debut=debut+step){
	tmp=quotesubst(v[0],v[1],debut,contextptr);
	res.push_back(quotesubst(eval(tmp,contextptr),v[1],debut,contextptr));
      }
    }
  ...
  identificateur independant_identificateur(const gen & g){
  ...
  // type=0 for seq, 1 for prod, 2 for sum
  gen seqprod2(const gen & g,int type,GIAC_CONTEXT){
    vecteur v(gen2vecteur(g));
    if (v.size()==1)
  ...
  gen _sum(const gen & args,GIAC_CONTEXT) {
	...
	return _sum(gen(makevecteur(tmp,v[1],-v[3],-v[2])),contextptr);
      }
	...
	bool savequote=*it->_IDNTptr->quoted;
	*it->_IDNTptr->quoted=true;
	f=eval(f,eval_level(contextptr),contextptr);    
  ...
  const string _sum_s("sum");
  unary_function_eval __sum(&_sum,_sum_s);
  unary_function_ptr at_sum (&__sum,_QUOTE_ARGUMENTS,true);

  const string _somme_s("somme");
  unary_function_eval __somme(&_sum,_somme_s);
  unary_function_ptr at_somme (&__somme,_QUOTE_ARGUMENTS,true);
  ...
  */ 
  const string _add_s("add");
  unary_function_eval __add(&_sum //&_add
			    ,_add_s);
  unary_function_ptr at_add (&__add,_QUOTE_ARGUMENTS,true);
  ...
  const string _bernoulli_s("bernoulli");
  unary_function_unary __bernoulli(&_bernoulli,_bernoulli_s);
  unary_function_ptr at_bernoulli (&__bernoulli,0,true);
  ...
  int gsl_odesolve_function(double t, const double y[], double dydt[], void * params){
  ...
  int gsl_odesolve_jacobian (double t, const double y[], double * dfdy, double dfdt[], void * params){
    // compute
    odesolve_param * par =(odesolve_param *) params;
    try {
  ...
  gen odesolve(const gen & t0orig,const gen & t1orig,const gen & f,const gen & y0orig,double tstep,bool return_curve,double * ymin,double * ymax,int maxstep,GIAC_CONTEXT){
    ...
    if (signed(par->odesolve_f.size())!=dim)
      setdimerr();
    par->odesolve_fy=*derive(par->odesolve_f,yv,contextptr)._VECTptr;
    par->odesolve_ft=*derive(par->odesolve_f,t_id,contextptr)._VECTptr;
	...
	setsizeerr("RK8 evolve not successfull");
      if (return_curve)  {
  ...
  const string _odesolve_s("odesolve");
  unary_function_eval __odesolve(&_odesolve,_odesolve_s);
  unary_function_ptr at_odesolve (&__odesolve,0,true);
  ...
  const string _ibpdv_s("ibpdv");
  unary_function_eval __ibpdv(&_ibpdv,_ibpdv_s);
  unary_function_ptr at_ibpdv (&__ibpdv,0,true);
  ...
  gen fourier_an(const gen & f,const gen & x,const gen & T,const gen & n,const gen & a,GIAC_CONTEXT){
    ...
    if (n!=0) 
      an=2*an;
    return recursive_normal(an,contextptr);
  }
  gen _fourier_an(const gen & args,GIAC_CONTEXT){
    if (args.type!=_VECT) setsizeerr();
    vecteur v(*args._VECTptr);
    if (v.size()==4) v.push_back(0);
    if (v.size()!=5) setsizeerr();
    gen f=v[0],x=v[1],T=v[2],n=v[3],a=v[4];
    return fourier_an(f,x,T,n,a,contextptr);
  }
  const string _fourier_an_s("fourier_an");
  unary_function_eval __fourier_an(&_fourier_an,_fourier_an_s);
  unary_function_ptr at_fourier_an (&__fourier_an,0,true);
  ...
  gen fourier_bn(const gen & f,const gen & x,const gen & T,const gen & n,const gen & a,GIAC_CONTEXT){
    ...
    return recursive_normal(2*iT*primi,contextptr);
  }
  ...
  const string _fourier_bn_s("fourier_bn");
  unary_function_eval __fourier_bn(&_fourier_bn,_fourier_bn_s);
  unary_function_ptr at_fourier_bn (&__fourier_bn,0,true);
  ...
  gen fourier_cn(const gen & f,const gen & x,const gen & T,const gen & n,const gen & a,GIAC_CONTEXT){
    ...
    gen cn=recursive_normal(iT*primi,contextptr);
    return cn;
  }
  ...
  const string _fourier_cn_s("fourier_cn");
  unary_function_eval __fourier_cn(&_fourier_cn,_fourier_cn_s);
  unary_function_ptr at_fourier_cn (&__fourier_cn,0,true);
