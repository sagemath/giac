// -*- mode:C++ ; compile-command: "g++-3.4 -I.. -I../include -g -c -Wall modpoly.cc" -*-
#include "first.h"
...
namespace giac {
  ...
  */
  ...
  bool is_rational(double d,int & num,int & den,double eps){
    double dcopy(d);
  ...
  modpoly modularize(const polynome & p,const gen & n,environment * env){
    bool ismod;
    if (env->coeff.type!=_USER && !is_zero(n)){
      env->modulo=n;
      env->pn=env->modulo;
      ismod=true;
      env->moduloon=true;
    ...
    gen n0(0);
    vecteur v;
    if (p.dim!=1) setsizeerr("modpoly.cc/modularize");
    if (p.coord.empty())
      return v;
    ...
    for (;it!=itend;++it){
      int newpow=it->index.iptr->front();
  ...
  vecteur x_to_xp(const vecteur & v, int p){
    if (p<=0) setsizeerr("modpoly.cc/x_to_xp");
    if ( (p==1) || v.empty())
      return v;
  ...
  void Muldense_POLY1(const modpoly::const_iterator & ita0,const modpoly::const_iterator & ita_end,const modpoly::const_iterator & itb0,const modpoly::const_iterator & itb_end,environment * env,modpoly & new_coord){
    mpz_t prod;
    mpz_init(prod);
    new_coord.clear();
	...
	add_mul(res,prod,*ita_cur,*itb_cur); // res = res + (*ita_cur) * (*itb_cur);
  ...
  void Mulmodpolymod(modpoly::const_iterator ita,modpoly::const_iterator ita_end,modpoly::const_iterator itb,modpoly::const_iterator itb_end,environment * env,modpoly & new_coord){
    int a=(ita_end-ita);
    int b=(itb_end-itb);
    if (!b)
      return ;
  ...
  void Mulmodpolysmall(modpoly::const_iterator & ita,modpoly::const_iterator & ita_end,modpoly::const_iterator & itb,modpoly::const_iterator & itb_end,environment * env,modpoly & new_coord){
    int a=(ita_end-ita);
    int b=(itb_end-itb);
    if (!b)
      return ;
  ...
  void mulmodpoly(const modpoly & a, const modpoly & b, environment * env,modpoly & new_coord){
    ...
    if (&a==&new_coord) setsizeerr("modpoly.cc/mulmodpoly");
  ...
  bool DivRem(const modpoly & th, const modpoly & other, environment * env,modpoly & quo, modpoly & rem,bool allowrational){
	...
	if (env && (env->moduloon && !env->complexe) && (env->modulo.type==_INT_) && (env->modulo.val<smallint)){
	  ...
	  mpz_init(prod);
	  for (;itq!=B_end;--tmpptr,++itq){ // no mod here to save comput. time
	    if (fast && (tmpptr->type==_ZINT) && (*(tmpptr->ptr_val.ref_count)==1) && ( (itq->type==_ZINT) || (itq->type==_INT_) ) )
  ...
  */
  ...
  void DivRem(const vector<int> & th, const vector<int> & other,int m,vector<int> & quo, vector<int> & rem){
    ...
    // make division of A*coeff by B*coeff and multiply R by coeff at the end
    vector<int> B=other;
    mulmodpoly(rem,invcoeff,m); // rem=th*invcoeff;
    mulmodpoly(B,invcoeff,m); // B=other*invcoeff;
    // copy rem to an array
  ...
  // compute a mod b into r
  // r, b must be allocated arrays of int
  void rem(int * & r,int *rend,int * b,int *bend,int m,int * & quo,int *quoend){
	...
	*qcur=q;
	++qcur;
      ...
      ++r;
      if (q){
	for (j=r,i=b;i!=bend;++j,++i){
	  //#ifdef _I386_   
	  //type_operator_plus_times_reduce(q,*i,*j,m);
	  //#else
	  *j -= q * *i;
	  *j %= m;
	  //#endif
  ...
  void gcdconvert(const modpoly & p,int m,int * a){
    const_iterateur it=p.begin(),itend=p.end();
    for (;it!=itend;++it,++a){
      if (it->type==_INT_)
	*a=it->val % m;
  ...
  bool gcdconvert(const polynome & p,int m,int * a){
  ...
  bool gcdsmallmodpoly(const polynome &p,const polynome & q,int m,polynome & d,polynome & dp,polynome & dq,bool compute_cof){
	...
	for (int deg=qend-qcur-1;qcur!=qend;++qcur,--deg){
	  if (*qcur)
	    dp.coord.push_back(monomial<gen>(*qcur,deg,1,1));
	}
	qcur=qsave;
	...
	for (int deg=qend-qcur-1;qcur!=qend;++qcur,--deg){
	  if (*qcur)
	    dq.coord.push_back(monomial<gen>(*qcur,deg,1,1));
	}
#ifdef VISUALC
	delete [] qsave; 
#endif
  ...
  void gcdsmallmodpoly(const vector<int> &p,const vector<int> & q,int m,vector<int> & d){
    ...
    int bsave[bs], *b=bsave,*bend=b+bs;
#endif
    memcpy(a,&p.front(),as*sizeof(int));
    memcpy(b,&q.front(),bs*sizeof(int));
    int * t;
    for (;b!=bend;){
      rem(a,aend,b,bend,m,qcur,0);
      t=a; a=b; b=t;
  ...
  void dbgp(const modpoly & a){
    cout << a << endl;
  }

  void content_mod(const polynome & p,vecteur & gcd,environment * env){
    vector< monomial<gen> >::const_iterator it=p.coord.begin(),itend=p.coord.end();
    for (;it!=itend;++it){
  ...
  polynome pevaln(const polynome & p,const gen & alpha,const gen & modulo,vector<int> * & degptr,bool estreel){
	...
	tmp=smallmod?hornermod(*it->value._VECTptr,a,m):hornermod(*it->value._VECTptr,alpha,modulo);
      else
  ...
  void divmod(polynome & p,const vecteur & v,environment * env){
    if (v.size()==1){
      if (!is_one(v.front())){
	if (!env || !env->moduloon)
	  setsizeerr();
  ...
  void pp_mod(polynome & p,vecteur & v,environment * env){
    content_mod(p,v,env);
    divmod(p,v,env);
  }

  // extract xn dependency as a modpoly
  void convert_xn(const polynome & p,polynome & res){
    int dim=p.dim;
    res.dim=dim-1;
    vector< monomial<gen> >::const_iterator it=p.coord.begin(),itend=p.coord.end();
    index_t old(dim,-1);
  ...
  void convert_back_xn(const polynome & p,polynome & res){
    res.coord.clear();
    int dim=p.dim,deg;
    res.dim=dim+1;
    vector< monomial<gen> >::const_iterator it=p.coord.begin(),itend=p.coord.end();
  ...
  */
  ...
  int degree_xn(const polynome & p){
    int res=1;
    vector< monomial<gen> >::const_iterator it=p.coord.begin(),itend=p.coord.end();
    for (;it!=itend;++it){
      if (it->value.type==_VECT)
  ...
  int find_nonzero(const modpoly & p,index_t & res){
    res.clear();
    const_iterateur it=p.begin(),itend=p.end();
  ...
  void make_modprimitive_xn(polynome & p,const gen & modulo,polynome & content){
  ...
  bool modgcd(const polynome &p_orig, const polynome & q_orig, const gen & modulo, polynome & d,polynome & pcofactor,polynome & qcofactor,bool compute_cofactors){
    ...
    // we are now interpolating G=gcd(p,q)*a poly/x1
    // such that the leading coeff of G is Delta
    vector<int> pdeg(pxn.degree()),qdeg(qxn.degree()); 
    int spdeg=0,sqdeg=0;
    for (int i=0;i<dim-1;++i){
      spdeg += pdeg[i];
      sqdeg += qdeg[i];
    }
    vector<int> delta=index_min(pdeg,qdeg);
    int e=0; // number of evaluations
    int alpha=0;
    if (debug_infolevel>1)
      cerr << "gcdmod find alpha dim " << d.dim << " " << clock() << endl;
  ...
  vecteur reverse_rsolve(const vecteur & v_orig,bool psron){
    if (v_orig.size()%2)
      setsizeerr("Argument must be a vector of even size"+gen(v_orig).print(context0));
    vecteur v(v_orig);
    reverse(v.begin(),v.end());
    int n=v.size()/2;
  ...
  void egcd(const vector<modpoly> & a,environment * env,vector<modpoly> & u){
    int n=a.size();
    if (n==0) setsizeerr("modpoly.cc/egcd");
    ...
    for (int k=0;k<=n-2;k++){
      ...
      if (!is_one(d)) setsizeerr("modpoly.cc/egcd");
      // multiply by v and U by c, _VECTute new c, push u[]
  ...
  void inpowmod(const modpoly & p,const gen & n,const modpoly & pmod,environment * env,modpoly & res){
    if (is_zero(n)){
      res=one();
      return ;
    }
  ...
  modpoly powmod(const modpoly & p,const gen & n,const modpoly & pmod,environment * env){
    if (!ck_is_positive(n,0)) setsizeerr("modpoly.cc/powmod");
    modpoly res;
  ...
  const string _horner_s("horner");
  unary_function_eval __horner(&giac::_horner,_horner_s);
  unary_function_ptr at_horner (&__horner,0,true);
  ...
  // better suited if x is symbolic
	...
	if (i==d)
	  ...
	  res=res+(*it)*symbolic(at_pow,makevecteur(x,i-d));
      }
    }
    return res;
  }
  ...
  modpoly taylordiff(const modpoly & p,const gen & x){
    int d=p.size();
    modpoly res(p),P(p);
    for (int i=1;i<=d;++i){
      res[d-i]=horner(P,x);
  ...
  bool giac_gcd_modular_algo1(polynome &p,polynome &q,polynome &d){
	...
	// m>gcddeg this prime is bad, just ignore
      ...
      if (is_strictly_greater(productmodulo,gen(gcddeg)*curpcofmax*curgcdmax,context0) &&
	  is_strictly_greater(productmodulo,gen(gcddeg)*curqcofmax*curgcdmax,context0) ){
	coeff=ppz(d);
	p=(currentpcof*coeff)/gcdlcoeff;
	q=(currentqcof*coeff)/gcdlcoeff;
	return true;
      }
    }
    return false;
  ...
  void ininttype2ZZ(const inttype & temp,const inttype & step,NTL::ZZ & z,const NTL::ZZ & zzstep){
    ...
    if (rem.type!=_INT_) setsizeerr("modpoly.cc/ininttype2ZZ");
  ...
  void polynome2tab(const polynome & p,int deg,inttype * tab){
    inttype n0(0);
    if (p.dim!=1) setsizeerr("modpoly.cc/polynome2tab");
    if (p.coord.empty())
      return ;
    if ( deg!=p.lexsorted_degree()) setsizeerr("modpoly.cc/polynome2tab");
    int curpow=deg;
    vector< monomial<gen> >::const_iterator it=p.coord.begin();
    vector< monomial<gen> >::const_iterator itend=p.coord.end();
    for (;it!=itend;++it){
