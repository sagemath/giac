// -*- mode:C++ ; compile-command: "g++-3.4 -I.. -g -c intgab.cc" -*-
#include "first.h"
/*
...
namespace giac {
  ...
  int is_meromorphic(const gen & g,const gen & x,gen & a,gen & b,gen & P,GIAC_CONTEXT){
	...
	return 0;
      ...
      if (v.size()==2) {
	if (v[1].is_symb_of_sommet(at_exp) && is_linear_wrt(P,*x._IDNTptr,a,b,contextptr)){
	...
	if ( (v[1].is_symb_of_sommet(at_cos) || v[1].is_symb_of_sommet(at_sin)) && is_linear_wrt(P,*x._IDNTptr,a,b,contextptr) && is_zero(im(a,contextptr)) )
	  return 3;
	...
	if (v[1].is_symb_of_sommet(at_cos) && v[2].is_symb_of_sommet(at_sin)){
	  ...
	  if (P==v2f && is_linear_wrt(P,*x._IDNTptr,a,b,contextptr) && is_zero(im(a,contextptr)))
	    return 3;
	}
      }
  ...
  int is_even_odd(const gen & f,const gen & x,GIAC_CONTEXT){
    gen f1=normal(_texpand(f,contextptr),contextptr),f2=normal(_texpand(subst(f,x,-x,false,contextptr),contextptr),contextptr);
    if (f1==f2)
      return 1;
    if (is_zero(f1+f2))
      return 2;
  ...
  gen residue(const gen & g,const gen & x,const gen & a,GIAC_CONTEXT){
	...
	if (is_strictly_positive(e1,contextptr)){
	  ...
	  if (is_zero(derive(res,x,contextptr)))
	    return 0;
	  setsizeerr("Non holomorphic function "+g.print(contextptr)+" at "+x.print(contextptr)+"="+a.print(contextptr));
	}
  ...
  gen _residue(const gen & args,GIAC_CONTEXT){
	...
	setsizeerr("Syntax residue(expr,x=a)");
      ++s;
    }
    if (s<3)
      setsizeerr();
  ...
  const string _residue_s("residue");
  unary_function_eval __residue(&_residue,_residue_s);
  unary_function_ptr at_residue (&__residue,0,true);
  ...
  const string _singular_s("singular");
  unary_function_eval __singular(&_singular,_singular_s);
  unary_function_ptr at_singular (&__singular,0,true);

  void assume_t_in_ab(gen & t,const gen & a,const gen & b,bool exclude_a,bool exclude_b,GIAC_CONTEXT){
    ...
    sto(gen(makevecteur(gen(_DOUBLE_).change_subtype(1),v_interval,v_excluded),_ASSUME__VECT),t,contextptr);
  }

  // reduce g, a rational fraction wrt to x, to a sqff lnpart
  // and adds the non sqff integrated part to ratpart
  bool intreduce(const gen & e,const gen & x,gen & lnpart,gen & ratpart,GIAC_CONTEXT){
    vecteur l;
  ...
  bool intgab_sincos(const gen & g,const gen & x,const gen & a,const gen & b,gen & A, gen & B,gen & res,GIAC_CONTEXT){
	...
	Dp=Dp*C;
      ...
      if (!Q.coord.empty() && is_zero(*Q.coord.back().index.iptr))
	...
	if (sturmab(vdp[i],XX,-1,1,contextptr)>0){
      ...
      factorization fd;
      polynome Dp_content;
      if (!factor(Dp,Dp_content,fd,false,true,true)){
  ...
  int nvars_depend_x(const vecteur & v,const gen & x){
    int res=0;
  ...
  bool intgab(const gen & g0,const gen & x,const gen & a,const gen & b,gen & res,GIAC_CONTEXT){
	...
	if (diracorder<0)
	  setsizeerr("Negative second Dirac argument");
	A=derive(A,x,diracorder,contextptr);
	if (diracorder%2)
	  A=-A;
	heav=heav._VECTptr->front();
      }
      if (!is_linear_wrt(heav,*x._IDNTptr,a0,b0,contextptr) || is_zero(a0))
	return false;
	...
	*logptr(contextptr) << "Warning, Dirac function outside summation interval" << endl;
    ...
    int s1=nvars_depend_x(lop(g,sincostan_v),x);
	...
	gen A,B,P;
	int typeint=is_meromorphic(g,x,A,B,P,contextptr);
	if (typeint==5){ 
	...
	if (typeint==2){ 
	  ...
	  try {
		...
		return false;
	      ...
	      assume_t_in_ab(gt,0,cst_pi,true,true,contextptr);
	    ...
	    assume_t_in_ab(gt,-cst_pi,0,true,true,contextptr);
	    gl=limit(glim,r,plus_inf,1,contextptr);
	...
	return false;
      ...
      if (eo==1 && intgab(gb,x,a,plus_inf,res,contextptr)){
	res=ratnormal(res/2);
	return true;
      }
      if (eo==2){
    ...
    if (b==plus_inf){
      ...
      if (eo==1 && intgab(ga,x,minus_inf,plus_inf,res,contextptr)){
	if (!is_inf(res))
	  res=ratnormal(res/2);
	return true;
      }
      vecteur v;
      rlvarx(ga,*x._IDNTptr,v);
      int vs=v.size();
      for (int i=0;i<vs;++i){
	if (v[i].is_symb_of_sommet(at_ln)){
	  gen f=v[i]._SYMBptr->feuille,a,b;
    ...
    gen gab=simplify(subst(g0,x,b,false,contextptr)-subst(g0,x,a,false,contextptr),contextptr);
    gen gm=subst(g0,x,b,false,contextptr)+subst(g0,x,a,false,contextptr);
    int eo=0;
    if (is_zero(gab) || is_zero(simplify(gm,contextptr)) ){
	...
	if (s>=2){
	  ...
	  for (i=0;i<s;++i){
	    if (v[i].is_symb_of_sommet(at_exp)){
	      ...
	      if (is_linear_wrt(v0arg,*x._IDNTptr,alphacur,betacur,contextptr) && is_integer( (periodecur=normal(alphacur*(b-a)/cst_two_pi/cst_i,contextptr)) )){
	  ...
	  if (!is_zero(periode)){
	    ...
	    *logptr(contextptr) << "Searching int of " << gof << " where " << x << " is on the unit circle, using residues" << endl;
	    ...
	    for (int i=0;i<s;++i){
	      ...
	      if (is_one(wabs) && !is_zero(residue(gof,x,w[i],contextptr))){
		res = unsigned_inf;
		return true;
	      }
	      if (is_strictly_greater(1,wabs,contextptr)){
  ...
  gen quotesubstcheck(const gen & g,const gen & x,const gen & i,const vecteur & v,GIAC_CONTEXT){
    if (!equalposcomp(v,i))
  ...
  bool sumab_int(gen & tmp1,const gen & gx,const vecteur & decals,const gen & lcoeff,GIAC_CONTEXT){
	...
	tmp1=integrate(tmp1,*gx._IDNTptr,remains,contextptr);
  ...
  bool in_sumab(const gen & g,const gen & x,const gen & a_orig,const gen & b_orig,gen & res,GIAC_CONTEXT){
    if (x.type!=_IDNT)
      return false;
    if (is_zero(g)){
      res=zero;
	...
	++it;
	if (!sumab(axb,*x._IDNTptr,a_orig,b_orig,tmp,contextptr)){
	  return false;
	}
	res += tmp;
      }
    ...
    if (!est_reel){
      ...
      if (!in_sumab(reg,x,a_orig,b_orig,reres,contextptr) || !in_sumab(img,x,a_orig,b_orig,imres,contextptr))
	return false;
      res=reres+cst_i*imres;
      return true;
    }
    if (b==plus_inf && a.type==_INT_ && is_hypergeometric(g,*x._IDNTptr,v,p,q,r,contextptr)){
      polynome s,Q,R;
	...
	if (is_admissible_poly(s,intstep,lcoeffs,decals,contextptr)){
	  ...
	  gen gx(idx);
	  assume_t_in_ab(gx,-1,1,true,true,contextptr);
	  ...
	  if (r==1){
	    ...
	    index_t ind=*R.coord.front().index.i();
	    ind[0]=0;
	  ...
	  if (!r){
	    ...
	    if (!is_zero(remains))
	      setsizeerr("Unable to sum "+remains.print(contextptr));
  ...
  // if true put int(g,x=a..b) into res
  // a or b must be +/-infinity and a<b
  bool sumab(const gen & g,const gen & x,const gen & a_orig,const gen & b_orig,gen & res,GIAC_CONTEXT){
    if (x.type!=_IDNT)
      return false;
    if (g.is_symb_of_sommet(at_plus)){
      ...
      gen tmp;
      for (i=0;i<args;++i){
	if (!sumab(argv[i],x,a_orig,b_orig,tmp,contextptr))
	  break;
	res += tmp;
      }
      if (i==args)
	...
	heav=heav._VECTptr->front();
      ...
      if (!is_linear_wrt(heav,*x._IDNTptr,a,b,contextptr) || is_zero(a))
	return false;
      if (!sumab(B,x,a_orig,b_orig,res,contextptr))
	return false;
      gen c=-b/a;
	...
	*logptr(contextptr) << "Warning, Dirac function outside summation interval" << endl;
    ...
    if (v.empty())
      return in_sumab(g,x,a_orig,b_orig,res,contextptr);
    gen A,B,a,b;
    identificateur t(" tsumab");
    gen h=quotesubst(g,v.front(),t,contextptr);
    if (!is_linear_wrt(h,t,A,B,contextptr))
    ...
    // A*Heaviside()+B
    gen heav=v.front()._SYMBptr->feuille;
    if (!is_linear_wrt(heav,*x._IDNTptr,a,b,contextptr))
      return false;
    if (!sumab(B,x,a_orig,b_orig,res,contextptr))
      return false;
    ...
    gen sumA;
    if (!sumab(A,x,newa,newb,sumA,contextptr))
      return false;
    res += sumA;
    return true;
  }
