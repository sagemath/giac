...
namespace giac {
  ...
  vector<localized_string> find_synonymes(const std::string & cmd_name){
  ...
  void readhelp(vector<aide> & v,const char * f_name,int & count,bool warn){
	...
	continue;
      ...
      // look for space
      int l=current.find_first_of(' ');
      if ( (l==1) && (current[0]=='0') ){
	int cs=current.size();
	while (l<cs && current[l]==' '){ ++l; }
        current_aide.syntax=current.substr(l,cs-l);
        continue;
      }
      int n=0;
      bool positif=true;
	...
	if ((current[i]<'0') || (current[i]>'9')){
	  ...
	  n=10*n+(current[i]-int('0'));
      }
      if (!positif)
	n=-n;
      if (n>0)
	current_blabla.push_back(localized_string(n,current.substr(l+1,current.size()-l)));
      else {
	if (n<0)
	  current_related.push_back(indexed_string(-n,current.substr(l+1,current.size()-l)));
	else
...
#ifdef __APPLE__
	if (access("/Volume/xcas/xcas",R_OK)==0)
	  xcasroot="/Volume/xcas/";
#else
  ...
  bool get_index_from_cache(const char * filename, multimap<string,string> & multi,bool verbose){
#ifdef VISUALC
  ...
  bool get_index_from_cache(const char * filename, vector<string> & multi,bool verbose){
#ifdef VISUALC
    char * buf = new char[BUFFER_SIZE];
#else
    char buf[BUFFER_SIZE];
  ...
  bool multigrep(FILE * f,const string & s){
    int l=s.size();
    // find spaces
    string tmp;
    vector<string> vs;
  ...
  static char otherchars[]="_.~µáàäãâéèêëîíìïóòöôõúùüûýÿñÀÁÃÂÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÝç½\0";
