...
namespace giac {
  ...
  gen integrate_without_lnabs(const gen & e,const gen & x,GIAC_CONTEXT){
    ...
    gen res=integrate(e,x,contextptr);
  ...
  gen laplace(const gen & f0,const gen & x,const gen & s,GIAC_CONTEXT){
    ...
    if (ffden.type==_POLY){
      ...
      if (!ffdenp.coord.empty() && (n=ffdenp.coord.back().index.iptr->front()) ){
  ...
  gen ilaplace(const gen & f,const gen & x,const gen & s,GIAC_CONTEXT){
    if (x.type!=_IDNT)
      setsizeerr();
    gen remains,res=linear_apply(f,*x._IDNTptr,remains,contextptr,pf_ilaplace);
  ...
  void unable_to_solve_diffeq(){
    setsizeerr("Unable to solve differntial equation");
  }
  ...
  int diffeq_order(const gen & f,const gen & y){
    vecteur ydepend(rlvarx(f,*y._IDNTptr));
  ...
  bool is_linear_diffeq(const gen & f_orig,const gen & x,const gen & y,int order,vecteur & v,GIAC_CONTEXT){
    v.clear();
  ...
  void find_n_derivatives_function(const gen & f,const gen & x,int & nder,gen & fonction){
    ...
    if (f._SYMBptr->feuille.type!=_VECT){
      find_n_derivatives_function(f._SYMBptr->feuille,x,nder,fonction);
      ++nder;
      return;
    }
    ...
    if ( (v.size()>1) && (v[1]!=x) )
      setsizeerr();
    find_n_derivatives_function(v[0],x,nder,fonction);
  ...
  gen function_of(const gen & y_orig,const gen & x_orig){
  ...
  gen in_desolve_with_conditions(const vecteur & v,const gen & x,const gen & y,const gen & solution_generale,const vecteur & parameters,GIAC_CONTEXT){
    gen yy(y);
    if (yy.type!=_IDNT)
      yy=function_of(y,x);
    ...
    vecteur condvar(rlvarx(conditions,*yy._IDNTptr));
	...
	continue;
      ...
      int nder;
      gen fonction;
      find_n_derivatives_function(w[0],x,nder,fonction);
      if (fonction==y){
    ...
    bool save_maple_mode=xcas_mode(contextptr);
  ...
  gen desolve_with_conditions(const vecteur & v,const gen & x,const gen & y,GIAC_CONTEXT){
    if (v.empty())
      setsizeerr();
    int ordre;
    vecteur parameters;
  ...
  bool separate_variables(const gen & f,const gen & x,const gen & y,gen & xfact,gen & yfact,GIAC_CONTEXT){
    ...
    for (;jt!=jtend;jt+=2){
      vecteur tmp(*_lname(*jt)._VECTptr);
  ...
  gen desolve(const gen & f_orig,const gen & x_orig,const gen & y_orig,int & ordre,vecteur & parameters,GIAC_CONTEXT){
    ...
    if (n==1) { // 1st order 
      ...
      if (x.type==_IDNT && y.type==_IDNT && is_linear_wrt(f,*y._IDNTptr,fc,fd,contextptr) && is_linear_wrt(fd,*x._IDNTptr,fa,fb,contextptr)){
	...
	// homogeneous?
	gen tplus(t);
	sto(doubleassume_and(vecteur(2,0),0,1,false,contextptr),tplus,contextptr);
	...
	if (is_zero(f)){
	  *logptr(contextptr) << "Homogeneous" << endl;
	  sto(doubleassume_and(vecteur(2,0),0,1,false,contextptr),x,contextptr);
  ...
  unary_function_eval __desolve(&_desolve,_desolve_s);
  unary_function_ptr at_desolve (&__desolve,1,true);

  const string _dsolve_s("dsolve");
  ...
  gen ztrans(const gen & f,const gen & x,const gen & s,GIAC_CONTEXT){
    ...
    if (s==x)
      t=identificateur(" t");
    assume_t_in_ab(t,1,plus_inf,true,true,contextptr);
