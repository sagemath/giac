...
namespace giac {
  ...
  void roots(const modpoly & q,environment * env, vecteur & v,vector<modpoly> & w){
    modpoly ddfactor(q);
    modpoly x(xpower1());
    normalize_env(env);
  ...
  void xtoxpowerpn(const modpoly & r, const vector<modpoly> & v, environment * env,int qsize,modpoly & s){
    s.clear();
    if (r.empty())
      return ;
    normalize_env(env);
  ...
  gen lastnonzero(const dense_POLY1 & q) {
    int d=q.size();
    gen n( 0),n0( 0);
    for (;d;--d){
      n=q[d-1];
  ...
  void ddf(const modpoly & q,const vector<modpoly> & qmat,environment * env,vector< facteur<modpoly> >& v){
	...
	xtoxpowerpn(operator_mod(xpi,qrem,env),qmat,env,0,xpi);
  ...
  void cantor_zassenhaus(const modpoly & ddfactor,int i,const vector<modpoly> & qmat,environment * env,vector<modpoly> & v){
    if (debuglevel)
	...
	for (unsigned ii=1;ii<m;ii *=2){
	  xtoxpowerpn(ppp,thisqmat,env,k,temp);
	  ppp=temp;
    ...
    cantor_zassenhaus(fact1,i,thisqmat,env,v); // recursive calls
    cantor_zassenhaus(operator_div(ddfactor,fact1,env),i,thisqmat,env,v);
  }

  void cantor_zassenhaus(const vector< facteur<modpoly> > & v_in,const vector<modpoly> & qmat, environment * env,vector<modpoly> & v){
    // split elements of v_in
    // cout << v_in << endl;
    vector< facteur<modpoly> >::const_iterator it=v_in.begin(),itend=v_in.end();
    for (;it!=itend;++it)
      cantor_zassenhaus(it->fact,it->mult,qmat,env,v);
  }
  ...
  void liftroots(const dense_POLY1 &q,environment * env,vecteur & v){
    // env->moduloon=false;
  ...
  void print_possible_degrees(const vector<bool> & possible_degrees,int qdeg){
  ...
  void extracttruefactors(dense_POLY1 & q, environment * env,vector<modpoly> & v_in, vector<modpoly> & v_orig, vectpoly & v_out, int & n,const gen & modulo_orig, const gen & moduloi, gen & bound, vector<modpoly> & u){
  ...
  int liftq(environment * env,dense_POLY1 & q,gen &bound,vector<modpoly> & v_in,vectpoly & v_out,vector<bool> & possible_degrees){
    if (debuglevel)
	...
	for (int i=0;itv!=itvend;++itv,++itu,++itv0,++i){
	  if (!truefactor[i]){
	    ...
	    debut_time=clock();
	    if (debuglevel)
	      cout << debut_time << "(" << CLOCKS_PER_SEC << ")" << "Searching true factor" << endl;
  ...
  // modulo is modified to modulo^k
  void liftl(environment * env,dense_POLY1 & q,gen &bound,vector<modpoly> & v_in,vectpoly & v_out){
    gen lcoeff(q.front());
    bool notunit=(!is_one(lcoeff));
    gen modulo_orig(env->modulo),moduloi(env->modulo),modulonext(env->modulo);
    vector<modpoly> v_orig(v_in),u;
	...
	extracttruefactors(q,env,v_in,v_orig,v_out,n,modulo_orig,moduloi,bound,u);
      if (!n)
	return;
      modulonext=moduloi*modulo_orig;
      env->moduloon=false;
  ...
  void combine(const dense_POLY1 & q, const vector<modpoly> & v_in,environment * env,vectpoly & v_out,vector<bool> & possible_degrees, int k=1){
    if (v_in.empty())
  ...
  int do_linearfind(const polynome & q,environment * env,polynome & qrem,vectpoly & v,vecteur & croots,int & i){
    ...
    if (q.dim!=1) setsizeerr("modfactor.cc/linearfind");
    ...
    if (i==100) setsizeerr("modfactor.cc/linearfind"); 
    vecteur w;
    vector<modpoly> wtmp;
    roots((pn>0 && signed(Q.size())>pn)?gcd(Q,operator_minus(xpowerpn(env),xpower1(),env),env):Q,env,w,wtmp);
  ...
  void do_factorunivsqff(const polynome & q,environment * env,vectpoly & v,int & i,int debug,int modfactor_primes){
    debuglevel=debug;  // 1 or debug;
    if (q.coord.empty())
      return ;
    if (q.dim!=1) setsizeerr("modfactor.cc/factorunivsqff");
    int qdeg=q.lexsorted_degree();
    if (!qdeg)
      return;
    if (qdeg==1){
    ...
    for (int essai=0;essai<modfactor_primes;i++){
      ...
      if (i==100){
	if (bestqmat.empty()) setsizeerr("modfactor.cc/factorunivsqff");
	break;
      }
    ...
    vector<modpoly> w;
    cantor_zassenhaus(wf,bestqmat,env,w);
    ...
    if (w.size()>12){
      ...
      pari_lift_combine(q1,w,env->modulo,res);
  ...
  void factorunivsqff(const polynome & q,environment * env,vectpoly & v,int & i,int debug=0,int modfactor_primes=MODFACTOR_PRIMES){
#ifdef HAVE_LIBPTHREAD
    int locked=pthread_mutex_trylock(&ntl_mutex);
#endif // HAVE_LIBPTHREAD
    if (locked){
      do_factorunivsqff(q,env,v,i,debug,modfactor_primes);
      return;
    }
    try {
  ...
  void factorunivsqff(const polynome & q,environment * env,vectpoly & v,int & i,int debug,int modfactor_primes){
    do_factorunivsqff(q,env,v,i,debug,modfactor_primes);
  }
