/* -*- mode:Mail; compile-command: "bison -p giac_yy -y -d input_parser.yy ; mv -f y.tab.c input_parser.cc ; mv -f y.tab.h input_parser.h ; g++ -g -c -I.. -I. input_parser.cc" -*- 
 *
...
%token 	T_NUMBER T_SYMBOL T_LITERAL T_DIGITS T_STRING T_END_INPUT
	...
	T_INTERROGATION T_UNIT T_BIDON T_LOGO T_SQ
...
exp	: T_NUMBER		{$$ = $1;}
	| T_NUMBER T_SYMBOL 	{$$=symbolic(at_prod,makevecteur($1,$2));}
	| T_NUMBER T_SYMBOL T_POW exp	{$$=symbolic(at_prod,makevecteur($1,symb_pow($2,$4)));}
	| T_NUMBER T_SYMBOL T_SQ	{$$=symbolic(at_prod,makevecteur($1,symb_pow($2,$3)));}
	| T_NUMBER T_LITERAL 	{$$=symbolic(at_prod,makevecteur($1,$2));}
	| T_STRING		{ $$=$1; }
	...
	| symbol T_BEGIN_PAR suite T_END_PAR T_AFFECT exp {$$ = symb_program_sto($3,$3*zero,$6,$1,true,giac_yyget_extra(scanner));}
	| exp TI_STO symbol T_BEGIN_PAR suite T_END_PAR {$$ = symb_program_sto($5,$5*zero,$1,$3,false,giac_yyget_extra(scanner));}
  ...
  $$ =symbolic(*$2._FUNCptr,makevecteur($1,$3));}
	...
	/* 
	| T_TI_IF exp T_THEN bloc T_ELSE bloc {$$ = symbolic(*$1._FUNCptr,makevecteur(equaltosame($2),symb_bloc($4),symb_bloc($6)));} 
 	| T_TI_IF exp TI_DEUXPOINTS exp {$$ = symbolic(*$1._FUNCptr,makevecteur(equaltosame($2),$4,undef));} 
	| T_TI_IF exp T_THEN prg_suite elif {
	...
	*/
	...
	| T_FOR symbol from T_TO exp step T_DO prg_suite T_BLOC_END { 
          gen tmp; 
          const giac::context * contextptr = giac_yyget_extra(scanner);
          if (!has_evalf($6,tmp,1,contextptr) || is_positive(tmp,contextptr)) 
             $$=symbolic(*$1._FUNCptr,makevecteur(symb_sto($3,$2),symb_inferieur_egal($2,$5),symb_sto(symb_plus($2,$6),$2),symb_bloc($8))); 
          else 
	...
	| T_FOR symbol from step T_TO exp T_DO prg_suite T_BLOC_END { 
         gen tmp; 
          const giac::context * contextptr = giac_yyget_extra(scanner);
         if (!has_evalf($6,tmp,1,contextptr) || is_positive(tmp,contextptr)) 
           $$=symbolic(*$1._FUNCptr,makevecteur(symb_sto($3,$2),symb_inferieur_egal($2,$6),symb_sto(symb_plus($2,$4),$2),symb_bloc($8))); 
         else 
	...
	| T_REPEAT prg_suite T_UNTIL exp { 
        ...
        v.push_back(symb_ifte(equaltosame($4),symbolic(at_break,zero),undef));
...
/*	| T_SYMBOL T_DOUBLE_DEUX_POINTS T_TYPE_ID { 
	       gen tmp($3); 
	       // tmp.subtype=1; 
	       $$=symb_check_type(makevecteur(tmp,$1)); 
          }
	...
	| T_SYMBOL T_DOUBLE_DEUX_POINTS exp { 
	   ...
	   $$=symb_check_type(makevecteur($3,$1)); 
        }
        else
...
entete	: /* empty */ { $$=vecteur(0); }
	| entete local	{ $$=mergevecteur(*$1._VECTptr,*$2._VECTptr); }
	| nom entete { $$=$2; }
	;
...
local	: T_LOCAL suite_symbol T_SEMI  { if (!$1.val) $$=$2; else $$=vecteur(0);}
	;
...
suite_symbol : affectable_symbol { $$=gen(vecteur(1,$1),_SEQ__VECT); }
	     | affectable_symbol T_VIRGULE suite_symbol { 
	       vecteur v=*$3._VECTptr;
	       v.insert(v.begin(),$1);
	       $$=gen(v,_SEQ__VECT);
	     }
	     ;
