...
namespace giac {
  ...
  bool operator < (const order_vectpoly & ov1,const order_vectpoly & ov2){
  ...
  std::map<order_vectpoly,Qx_I> cocoa_idealptr_map;

  CoCoA::GlobalManager CoCoAFoundations;

  CoCoA::ZZ gen2ZZ(const gen & g){
    switch (g.type){
    case _INT_:
  ...
  gen ZZ2gen(const CoCoA::RingElem & z){
    CoCoA::ZZ n,d;
    ...
    setsizeerr("Unable to convert CoCoA data");
    return undef;
  }

  CoCoA::RingElem polynome2ringelem(const polynome & p,const std::vector<CoCoA::RingElem> & x){
    ...
    for (;it!=itend;++it){
      ...
      index_t::const_iterator jt=it->index.iptr->begin(),jtend=it->index.iptr->end();
  ...
  polynome ringelem2polynome(const CoCoA::RingElem & f){
  ...
  void vector_polynome2vector_ringelem(const vectpoly & v,const CoCoA::SparsePolyRing & Qx,vector<CoCoA::RingElem> & g){
  ...
  vector<CoCoA::RingElem> vector_polynome2vector_ringelem(const vectpoly & v,const CoCoA::SparsePolyRing & Qx){
    vector<CoCoA::RingElem> g;
    vector_polynome2vector_ringelem(v,Qx,g);
    return g;
  }

  void vector_ringelem2vector_polynome(const vector<CoCoA::RingElem> & g, vectpoly & res){
    vector<CoCoA::RingElem>::const_iterator it=g.begin(),itend=g.end();
    res.reserve(itend-it);
    for (;it!=itend;++it)
      res.push_back(ringelem2polynome(*it));
  }
  vectpoly vector_ringelem2vector_polynome(const vector<CoCoA::RingElem> & g){
    vectpoly res;
    vector_ringelem2vector_polynome(g,res);
    return res;
  }

  Qx_I get_or_make_idealptr(const vectpoly & v,const gen & order){
  ...
  void homogeneize(polynome & p,int deg){
