...
namespace giac {
  ...
  gen in_select_root(const vecteur & a,GIAC_CONTEXT){
    if (a.empty())
      return undef;
    gen current(a.front());
    double max_re(re(current,contextptr).evalf(1,contextptr)._DOUBLE_val),max_im(im(current,contextptr).evalf(1,contextptr)._DOUBLE_val);
    const_iterateur it=a.begin(),itend=a.end();
    for (;it!=itend;++it){
      double cur_re(re(*it,contextptr).evalf(1,contextptr)._DOUBLE_val),cur_im(im(*it,contextptr).evalf(1,contextptr)._DOUBLE_val);
      if (cur_re > max_re ){
	current=*it;
	max_re=cur_re;
	max_im=cur_im;
  ...
  gen ext_reduce(const gen & a, const gen & v){
    ...
    if (v.type!=_EXT)
      settypeerr("ext_reduce");
    gen va=*v._EXTptr,vb=*(v._EXTptr+1);
    if (va.type==_FRAC)
        return ext_reduce(horner(*a._VECTptr,*va._FRACptr),vb);
    if (va.type!=_VECT){
      if (vb.type!=_VECT)  setsizeerr("alg_ext.cc/ext_reduce");
      return algebraic_EXTension( (*a._VECTptr) % (*vb._VECTptr),v);
    }
    return ext_reduce(horner(*a._VECTptr,*va._VECTptr),vb);
  }

  gen ext_reduce(const gen & e){
#ifdef DEBUG_SUPPORT
    if (e.type!=_EXT)  setsizeerr("alg_ext.cc/ext_reduce");
#endif    
  ...
  void polynome2vecteur(const polynome & p,int na,int nb,vecteur & v){
    v=vecteur(na*nb,zero);
    int i,j;
    if (p.dim!=2)  setsizeerr("alg_ext.cc/polynome2vecteur");
    vector< monomial<gen> >::const_iterator it=p.coord.begin(),itend=p.coord.end();
    for (;it!=itend;++it){
      i=it->index.iptr->front();
      j=it->index.iptr->back();
  ...
  vecteur replace_ext(const vecteur & v,const vecteur &ext,const gen & g,GIAC_CONTEXT){
    vecteur res;
    const_iterateur it=v.begin(),itend=v.end();
	...
	if (*(numtmp._EXTptr+1)!=ext)
	  setsizeerr("Invalid _EXT in replace_ext");
  ...
  gen common_minimal_POLY(const gen & ga,const gen & gb, gen & a,gen & b,int & k,GIAC_CONTEXT){
	...
	polynome2vecteur(p,na,nb,ligne);
  ...
  vecteur min_pol(gen & a){
    if (a.type==_VECT)
      ...
      if ( (a.type!=_EXT) || ((a._EXTptr+1)->type!=_VECT) )  setsizeerr("alg_ext.cc/min_pol");
  ...
  gen common_EXT(gen & a,gen & b,GIAC_CONTEXT){
    ...
    if (a.type==_VECT)
      ...
      if ( (a.type!=_EXT) || ((a._EXTptr+1)->type!=_VECT) )  setsizeerr("alg_ext.cc/common_EXT");
      a__VECT=*(a._EXTptr+1);
    }
    if (b.type==_VECT)
      b__VECT=b;
    else {
      if ( (b.type!=_EXT) || ((b._EXTptr+1)->type!=_VECT) )  setsizeerr("alg_ext.cc/common_EXT");
      b__VECT=*(b._EXTptr+1);
    }
    int as=a__VECT._VECTptr->size(),bs=b__VECT._VECTptr->size();
    if (bs>as)
      return common_EXT(b,a,contextptr);
    ...
    if ((a_orig.type==_EXT) && (b_orig.type==_EXT))
  ...
  gen ext_add(const gen & aa,const gen & bb,GIAC_CONTEXT){
    ...
    gen c=common_EXT(*(a._EXTptr+1),*(b._EXTptr+1),contextptr);
  ...
  gen fminmax(const gen & g,int minmax,GIAC_CONTEXT){
	...
	setsizeerr("Or condition not implemented");
      range=*range.front()._VECTptr;
    }
    if (range.size()!=2)
      setsizeerr("fminmax, range "+gen(range).print(contextptr));
  ...
  void find_range(const gen & g,vecteur & a,GIAC_CONTEXT){
    ...
    if (g.type==_SYMB){
      try {
	vecteur lv(lvar(g._SYMBptr->feuille));
	if (!lv.empty()){
  ...
  int insturmsign(const gen & g0,GIAC_CONTEXT){
    gen g=normal(exact(g0,contextptr),contextptr);
    if (has_i(g))
      return 0;
    vecteur v(lvar(g));
	...
	if (is_zero(limit(testg,idnttmp,last,-1,contextptr))){
	  gen tmp=_e2r(makevecteur(gg,v0),contextptr);
	  gen den=plus_one;
	  if (tmp.type==_FRAC){
	    den=_r2e(makevecteur(tmp._FRACptr->den,v0),contextptr);
	    if (is_inf(last) && tmp._FRACptr->den.type==_VECT)
	      den=den/pow(v0,2*int(tmp._FRACptr->den._VECTptr->size()/2));
	  ...
	  if (tmp.type==_VECT){
	    ...
	    gg=_r2e(makevecteur(p,v0),contextptr)/den;
	  }
	}
	current_sign=sturmab(gg,v0,ita->_VECTptr->front(),last,contextptr);
	if (current_sign>0)
	  return 0;
	if (previous_sign==2)
	  previous_sign=current_sign;
  ...
  int sturmsign(const gen & g0,GIAC_CONTEXT){
    ...
    if (g.is_symb_of_sommet(at_prod)){
      ...
      for (int i=0;i<s;++i){
	if (v[i].is_symb_of_sommet(at_sqrt) && sturmsign(v[i]._SYMBptr->feuille,contextptr)==1)
	  continue;
	...
	return res;
      case 1:
	return res*insturmsign(w.front(),contextptr);
      default:
	return res*insturmsign(symbolic(at_prod,w),contextptr);
      }
    }
    return insturmsign(g,contextptr);
  }
