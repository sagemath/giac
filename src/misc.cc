/* -*- mode:C++ ; compile-command: "g++-3.4 -I.. -g -c misc.cc" -*- */
#include "first.h"
/*
...
namespace giac {
  ...
  const string _scalar_product_s("scalar_product");
  unary_function_eval __scalar_product(&_scalar_product,_scalar_product_s);
  unary_function_ptr at_scalar_product (&__scalar_product,0,true);

  const string _dot_s("dot");
  unary_function_eval __dot(&_scalar_product,_dot_s);
  unary_function_ptr at_dot (&__dot,0,true);

  gen _compare(const gen & args){
    if (args.type!=_VECT || args._VECTptr->size()!=2)
      setsizeerr();
    vecteur & v=*args._VECTptr;
    return v[0].islesscomplexthan(v[1]);
  }    
  const string _compare_s("compare");
  unary_function_unary __compare(&_compare,_compare_s);
  unary_function_ptr at_compare (&__compare,0,true);
  ...
  const string _preval_s("preval");
  unary_function_eval __preval(&_preval,_preval_s);
  unary_function_ptr at_preval (&__preval,0,true);
  ...
  const string _lagrange_s("lagrange");
  unary_function_eval __lagrange(&_lagrange,_lagrange_s);
  unary_function_ptr at_lagrange (&__lagrange,0,true);
  ...
  const string _reorder_s("reorder");
  unary_function_eval __reorder(&_reorder,_reorder_s);
  unary_function_ptr at_reorder (&__reorder,0,true);
  ...
  const string _adjoint_matrix_s("adjoint_matrix");
  unary_function_eval __adjoint_matrix(&_adjoint_matrix,_adjoint_matrix_s);
  unary_function_ptr at_adjoint_matrix (&__adjoint_matrix,0,true);

  gen _equal2diff(const gen & args){
    return apply(args,equal2diff);
  }    
  const string _equal2diff_s("equal2diff");
  unary_function_unary __equal2diff(&_equal2diff,_equal2diff_s);
  unary_function_ptr at_equal2diff (&__equal2diff,0,true);

  gen equal2list(const gen & arg){
    if ( (arg.type!=_SYMB) || (arg._SYMBptr->sommet!=at_equal))
      return makevecteur(arg,zero);
    return arg._SYMBptr->feuille;
  }
  gen _equal2list(const gen & args){
    return apply(args,equal2list);
  }    
  const string _equal2list_s("equal2list");
  unary_function_unary __equal2list(&_equal2list,_equal2list_s);
  unary_function_ptr at_equal2list (&__equal2list,0,true);
  ...
  const string _rank_s("rank");
  unary_function_eval __rank(&_rank,_rank_s);
  unary_function_ptr at_rank (&__rank,0,true);

  gen _sec(const gen & args,GIAC_CONTEXT){
    return inv(cos(args,contextptr),contextptr);
  }    
  const string _sec_s("sec");
  unary_function_eval __sec(&_sec,_sec_s);
  unary_function_ptr at_sec (&__sec,0,true);

  gen _csc(const gen & args,GIAC_CONTEXT){
    return inv(sin(args,contextptr),contextptr);
  }    
  const string _csc_s("csc");
  unary_function_eval __csc(&_csc,_csc_s);
  unary_function_ptr at_csc (&__csc,0,true);

  gen _cot(const gen & args,GIAC_CONTEXT){
    return rdiv(cos(args,contextptr),sin(args,contextptr));
  }    
  const string _cot_s("cot");
  unary_function_eval __cot(&_cot,_cot_s);
  unary_function_ptr at_cot (&__cot,0,true);

  gen _asec(const gen & args,GIAC_CONTEXT){
    return acos(inv(args,contextptr),contextptr);
  }    
  const string _asec_s("asec");
  unary_function_eval __asec(&_asec,_asec_s);
  unary_function_ptr at_asec (&__asec,0,true);

  gen _acsc(const gen & args,GIAC_CONTEXT){
    return asin(inv(args,contextptr),contextptr);
  }    
  const string _acsc_s("acsc");
  unary_function_eval __acsc(&_acsc,_acsc_s);
  unary_function_ptr at_acsc (&__acsc,0,true);

  gen _acot(const gen & args,GIAC_CONTEXT){
    return atan(inv(args,contextptr),contextptr);
  }    
  const string _acot_s("acot");
  unary_function_eval __acot(&_acot,_acot_s);
  unary_function_ptr at_acot (&__acot,0,true);
  ...
  gen _ibpu(const gen & args,GIAC_CONTEXT) {
    ...
    if (is_zero(v) || is_one(v)){
      gen tmp=integrate(uprimev,x,contextptr);
      if (bound)
	tmp=preval(tmp,x,a,b,contextptr);
      return tmp+F;
    }
    gen uprime(normal(rdiv(uprimev,v),contextptr));
    u=integrate(uprime,x,contextptr);
    F += u*v;
    if (bound)
      F = preval(F,x,a,b,contextptr);
    return makevecteur(F,normal(-u*derive(v,x,contextptr),contextptr));
  }
  const string _ibpu_s("ibpu");
  unary_function_eval __ibpu(&_ibpu,_ibpu_s);
  unary_function_ptr at_ibpu (&__ibpu,0,true);
  ...
  const string _changebase_s("changebase");
  unary_function_eval __changebase(&_changebase,_changebase_s);
  unary_function_ptr at_changebase (&__changebase,0,true);

  gen epsilon2zero(const gen & g,GIAC_CONTEXT){
    switch (g.type){
    case _DOUBLE_:
  ...
  const string _epsilon2zero_s("epsilon2zero");
  unary_function_eval __epsilon2zero(&_epsilon2zero,_epsilon2zero_s);
  unary_function_ptr at_epsilon2zero (&__epsilon2zero,_QUOTE_ARGUMENTS,true);
  ...
  const string _suppress_s("suppress");
  unary_function_eval __suppress(&_suppress,_suppress_s);
  unary_function_ptr at_suppress (&__suppress,0,true);

  int valuation(const polynome & p){
    if (p.coord.empty())
      return -1;
    return p.coord.back().index.iptr->front();
  }
  ...
  const string _valuation_s("valuation");
  unary_function_eval __valuation(&_valuation,_valuation_s);
  unary_function_ptr at_valuation (&__valuation,0,true);

  const string _ldegree_s("ldegree");
  unary_function_eval __ldegree(&_valuation,_ldegree_s);
  unary_function_ptr at_ldegree (&__ldegree,0,true);
  ...
  gen _degree(const gen & args,GIAC_CONTEXT){
    ...
    if (aan.type!=_POLY)
      return zero;
    return aan._POLYptr->lexsorted_degree();
  }    
  const string _degree_s("degree");
  unary_function_eval __degree(&_degree,_degree_s);
  unary_function_ptr at_degree (&__degree,0,true);
  ...
  gen _lcoeff(const gen & args,GIAC_CONTEXT){
	...
	n=nlcoeff;
      ...
      return _r2e(makevecteur(n/d,x),contextptr);
    }
    return n._VECTptr->front()/d;
  }
  const string _lcoeff_s("lcoeff");
  unary_function_eval __lcoeff(&_lcoeff,_lcoeff_s);
  unary_function_ptr at_lcoeff (&__lcoeff,0,true);

  gen tcoeff(const vecteur & v){
    int s=v.size();
    gen g;
    for (;s;--s){
      g=v[s-1];
  ...
  const string _tcoeff_s("tcoeff");
  unary_function_eval __tcoeff(&_tcoeff,_tcoeff_s);
  unary_function_ptr at_tcoeff (&__tcoeff,0,true);

  gen sqrfree(const gen & g,const vecteur & l,GIAC_CONTEXT){
    if (g.type!=_POLY)
  ...
  const string _sqrfree_s("sqrfree");
  unary_function_eval __sqrfree(&_sqrfree,_sqrfree_s);
  unary_function_ptr at_sqrfree (&__sqrfree,0,true);
  ...
  const string _truncate_s("truncate");
  unary_function_eval __truncate(&_truncate,_truncate_s);
  unary_function_ptr at_truncate (&__truncate,0,true);
  ...
  gen _canonical_form(const gen & args,GIAC_CONTEXT){
    ...
    if (x.type!=_IDNT)
      settypeerr();
    if (!is_quadratic_wrt(p,*x._IDNTptr,a,b,c,contextptr))
      setsizeerr();
    if (is_zero(a))
      return b*x+c;
  ...
  const string _canonical_form_s("canonical_form");
  unary_function_eval __canonical_form(&_canonical_form,_canonical_form_s);
  unary_function_ptr at_canonical_form (&__canonical_form,0,true);
  ...
  const string _fcoeff_s("fcoeff");
  unary_function_eval __fcoeff(&_fcoeff,_fcoeff_s);
  unary_function_ptr at_fcoeff (&__fcoeff,0,true);

  void addfactors(const gen & p,const gen & x,int mult,vecteur & res,GIAC_CONTEXT){
  ...
  const string _froot_s("froot");
  unary_function_eval __froot(&_froot,_froot_s);
  unary_function_ptr at_froot (&__froot,0,true);
  ...
  const string _roots_s("roots");
  unary_function_eval __roots(&_roots,_roots_s);
  unary_function_ptr at_roots (&__roots,0,true);
  ...
  const string _divpc_s("divpc");
  unary_function_eval __divpc(&_divpc,_divpc_s);
  unary_function_ptr at_divpc (&__divpc,0,true);
  ...
  const string _ptayl_s("ptayl");
  unary_function_eval __ptayl(&_ptayl,_ptayl_s);
  unary_function_ptr at_ptayl (&__ptayl,0,true);
  ...
  const string _dfc_s("dfc");
  unary_function_eval __dfc(&_dfc,_dfc_s);
  unary_function_ptr at_dfc (&__dfc,0,true);
  ...
  const string _dfc2f_s("dfc2f");
  unary_function_eval __dfc2f(&_dfc2f,_dfc2f_s);
  unary_function_ptr at_dfc2f (&__dfc2f,0,true);
  ...
  const string _float2rational_s("float2rational");
  unary_function_eval __float2rational(&_float2rational,_float2rational_s);
  unary_function_ptr at_float2rational (&__float2rational,0,true);
  ...
  const string _gramschmidt_s("gramschmidt");
  unary_function_eval __gramschmidt(&_gramschmidt,_gramschmidt_s);
  unary_function_ptr at_gramschmidt (&__gramschmidt,0,true);
  ...
  gen pmin(const matrice & m,GIAC_CONTEXT){
    int s=m.size();
    matrice mpow(midn(s));
    matrice res;
    vecteur v;
  ...
  gen _pmin(const gen & g,GIAC_CONTEXT){
    ...
    if (proba_epsilon(contextptr) && probabilistic_pmin(m,w,true,contextptr))
      ...
      return _r2e(makevecteur(pmin(m,contextptr),v.back()),contextptr);
  }
  const string _pmin_s("pmin");
  unary_function_eval __pmin(&_pmin,_pmin_s);
  unary_function_ptr at_pmin (&__pmin,0,true);
  ...
  gen _potential(const gen & g,GIAC_CONTEXT){
	...
	if (!is_zero(simplify(derive(fv[i],xv[j],contextptr)-derive(fv[j],xv[i],contextptr),contextptr)))
	  setsizeerr("Not a potential");
      }
    }
    gen res;
    for (int i=0;i<s;++i){
      res=res+integrate(simplify(fv[i]-derive(res,xv[i],contextptr),contextptr),xv[i],contextptr);
    }
    return res;
  }
  const string _potential_s("potential");
  unary_function_eval __potential(&_potential,_potential_s);
  unary_function_ptr at_potential (&__potential,_QUOTE_ARGUMENTS,true);
  ...
  gen _vpotential(const gen & g,GIAC_CONTEXT){
    ...
    if (!is_zero(simplify(_divergence(g,contextptr),contextptr)))
      setsizeerr("Not a vector potential");
    vecteur res(3);
    ...
    res[1]=integrate(fv[2],xv[0],contextptr);
    res[2]=integrate(fv[1],xv[0],contextptr);
    gen F=simplify(fv[0]+derive(res[1],xv[2],contextptr)+derive(res[2],xv[1],contextptr),contextptr);
    F=integrate(F,xv[1],contextptr);
    res[2]=F-res[2];
    return res;
  }
  const string _vpotential_s("vpotential");
  unary_function_eval __vpotential(&_vpotential,_vpotential_s);
  unary_function_ptr at_vpotential (&__vpotential,_QUOTE_ARGUMENTS,true);
  ...
  const string _poly2symb_s("poly2symb");
  unary_function_eval __poly2symb(&_poly2symb,_poly2symb_s);
  unary_function_ptr at_poly2symb (&__poly2symb,0,true);

  gen _symb2poly(const gen & g,GIAC_CONTEXT){
    return _e2r(g,contextptr);
  }
  const string _symb2poly_s("symb2poly");
  unary_function_eval __symb2poly(&_symb2poly,_symb2poly_s);
  unary_function_ptr at_symb2poly (&__symb2poly,0,true);

  gen _exp2trig(const gen & g,GIAC_CONTEXT){
    return _sincos(g,contextptr);
  }
  const string _exp2trig_s("exp2trig");
  unary_function_eval __exp2trig(&_exp2trig,_exp2trig_s);
  unary_function_ptr at_exp2trig (&__exp2trig,0,true);

  gen _nrows(const gen & g){
    if (!ckmatrix(g))
      setsizeerr();
    return int(g._VECTptr->size());
  }
  const string _nrows_s("nrows");
  unary_function_unary __nrows(&_nrows,_nrows_s);
  unary_function_ptr at_nrows (&__nrows,0,true);

  gen _ncols(const gen & g){
    if (!ckmatrix(g))
      setsizeerr();
    if (g._VECTptr->empty())
      return zero;
  ...
  const string _ncols_s("ncols");
  unary_function_unary __ncols(&_ncols,_ncols_s);
  unary_function_ptr at_ncols (&__ncols,0,true);
  ...
  const string _l2norm_s("l2norm");
  unary_function_eval __l2norm(&_l2norm,_l2norm_s);
  unary_function_ptr at_l2norm (&__l2norm,0,true);

  const string _norm_s("norm");
  unary_function_eval __norm(&_l2norm,_norm_s);
  unary_function_ptr at_norm (&__norm,0,true);

  gen _normalize(const gen & a,GIAC_CONTEXT){
    return a/_l2norm(a,contextptr);
  }
  const string _normalize_s("normalize");
  unary_function_eval __normalize(&_normalize,_normalize_s);
  unary_function_ptr at_normalize (&__normalize,0,true);

  const string _randmatrix_s("randmatrix");
  unary_function_eval __randmatrix(&_ranm,_randmatrix_s);
  unary_function_ptr at_randmatrix (&__randmatrix,0,true);

  extern unary_function_ptr at_lgcd;
  gen _lgcd(const gen & g){
    if (g.type!=_VECT)
      return symbolic(at_lgcd,g);
    return lgcd(*g._VECTptr);
  }
  const string _lgcd_s("lgcd");
  unary_function_unary __lgcd(&_lgcd,_lgcd_s);
  unary_function_ptr at_lgcd (&__lgcd,0,true);

  // synonyms
  gen _float(const gen & g,GIAC_CONTEXT){
    return evalf(g,1,contextptr);
  }
  const string _float_s("float");
  unary_function_eval __float(&_float,_float_s);
  unary_function_ptr at_float (&__float,0,true);

  gen _hold(const gen & g){
    return g;
  }
  const string _hold_s("hold");
  unary_function_unary __hold(&_hold,_hold_s);
  unary_function_ptr at_hold (&__hold,_QUOTE_ARGUMENTS,true);

  gen _eigenvals(const gen & g,GIAC_CONTEXT){
    return gen(*_diag(_egvl(g,contextptr))._VECTptr,_SEQ__VECT);
  }
  const string _eigenvals_s("eigenvals");
  unary_function_eval __eigenvals(&_eigenvals,_eigenvals_s);
  unary_function_ptr at_eigenvals (&__eigenvals,0,true);

  const string _giackernel_s("kernel");
  unary_function_eval __giackernel(&_ker,_giackernel_s);
  unary_function_ptr at_kernel (&__giackernel,0,true);

  const string _eigenvects_s("eigenvects");
  unary_function_eval __eigenvects(&_egv,_eigenvects_s);
  unary_function_ptr at_eigenvects (&__eigenvects,0,true);

  const string _eigenvalues_s("eigenvalues");
  unary_function_eval __eigenvalues(&_egvl,_eigenvalues_s);
  unary_function_ptr at_eigenvalues (&__eigenvalues,0,true);

  const string _charpoly_s("charpoly");
  unary_function_eval __charpoly(&giac::_pcar,_charpoly_s);
  unary_function_ptr at_charpoly (&__charpoly,0,true);

  const string _eigenvectors_s("eigenvectors");
  unary_function_eval __eigenvectors(&_egv,_eigenvectors_s);
  unary_function_ptr at_eigenvectors (&__eigenvectors,0,true);
  ...
  const string _multiply_s("multiply");
  unary_function_eval __multiply(&_prod,_multiply_s);
  unary_function_ptr at_multiply (&__multiply,0,true);

  /* Maple inert forms */
  gen _Gcd(const gen & g){
    return symbolic(at_gcd,g);
  }
  const string _Gcd_s("Gcd");
  unary_function_unary __Gcd(&_Gcd,_Gcd_s);
  unary_function_ptr at_Gcd (&__Gcd,0,true);

  gen _Gcdex(const gen & g){
    return symbolic(at_gcdex,g);
  }
  const string _Gcdex_s("Gcdex");
  unary_function_unary __Gcdex(&_Gcdex,_Gcdex_s);
  unary_function_ptr at_Gcdex (&__Gcdex,0,true);

  gen _Factor(const gen & g){
    return symbolic(at_factor,g);
  }
  const string _Factor_s("Factor");
  unary_function_unary __Factor(&_Factor,_Factor_s);
  unary_function_ptr at_Factor (&__Factor,0,true);

  gen _Rref(const gen & g){
    return symbolic(at_rref,g);
  }
  const string _Rref_s("Rref");
  unary_function_unary __Rref(&_Rref,_Rref_s);
  unary_function_ptr at_Rref (&__Rref,0,true);

  gen _Rank(const gen & g){
    return symbolic(at_rank,g);
  }
  const string _Rank_s("Rank");
  unary_function_unary __Rank(&_Rank,_Rank_s);
  unary_function_ptr at_Rank (&__Rank,0,true);

  gen _Det(const gen & g){
    return symbolic(at_det,g);
  }
  const string _Det_s("Det");
  unary_function_unary __Det(&_Det,_Det_s);
  unary_function_ptr at_Det (&__Det,0,true);

  gen _Quo(const gen & g){
    return symbolic(at_quo,g);
  }
  const string _Quo_s("Quo");
  unary_function_unary __Quo(&_Quo,_Quo_s);
  unary_function_ptr at_Quo (&__Quo,0,true);

  gen _Rem(const gen & g){
    return symbolic(at_rem,g);
  }
  const string _Rem_s("Rem");
  unary_function_unary __Rem(&_Rem,_Rem_s);
  unary_function_ptr at_Rem (&__Rem,0,true);

  gen _Int(const gen & g){
    return symbolic(at_integrate,g);
  }
  const string _Int_s("Int");
  unary_function_unary __Int(&_Int,_Int_s);
  unary_function_ptr at_Int (&__Int,0,true);

  gen _divisors(const gen & g){
    return gen(*_idivis(g)._VECTptr,_SET__VECT);
  }
  const string _divisors_s("divisors");
  unary_function_unary __divisors(&_divisors,_divisors_s);
  unary_function_ptr at_divisors (&__divisors,0,true);
  ...
  const string _maxnorm_s("maxnorm");
  unary_function_eval __maxnorm(&_maxnorm,_maxnorm_s);
  unary_function_ptr at_maxnorm (&__maxnorm,0,true);
  ...
  const string _l1norm_s("l1norm");
  unary_function_eval __l1norm(&_l1norm,_l1norm_s);
  unary_function_ptr at_l1norm (&__l1norm,0,true);
  ...
  const string _dotprod_s("dotprod");
  unary_function_eval __dotprod(&_dotprod,_dotprod_s);
  unary_function_ptr at_dotprod (&__dotprod,0,true);

  const string _crossproduct_s("crossproduct");
  unary_function_unary __crossproduct(&_cross,_crossproduct_s);
  unary_function_ptr at_crossproduct (&__crossproduct,0,true);

  gen _diag(const gen & g){
    if (g.type!=_VECT || g._VECTptr->empty())
      setsizeerr();
    vecteur v=*g._VECTptr;
    int l=v.size();
  ...
  const string _diag_s("diag");
  unary_function_unary __diag(&_diag,_diag_s);
  unary_function_ptr at_diag (&__diag,0,true);

  const string _BlockDiagonal_s("BlockDiagonal");
  unary_function_unary __BlockDiagonal(&_diag,_BlockDiagonal_s);
  unary_function_ptr at_BlockDiagonal (&__BlockDiagonal,0,true);

  gen _input(const gen & args,GIAC_CONTEXT){
    return _input(args,false,contextptr);
  }
  const string _input_s("input");
  unary_function_eval __input(&_input,_input_s);
  unary_function_ptr at_input (&__input,_QUOTE_ARGUMENTS,true);

  gen _textinput(const gen & args,GIAC_CONTEXT){
    return _input(args,true,contextptr);
  }
  const string _textinput_s("textinput");
  unary_function_eval __textinput(&_textinput,_textinput_s);
  unary_function_ptr at_textinput (&__textinput,_QUOTE_ARGUMENTS,true);

  const string _f2nd_s("f2nd");
  unary_function_unary __f2nd(&_fxnd,_f2nd_s);
  unary_function_ptr at_f2nd (&__f2nd,0,true);

  // service=-3 for content -2 for primpart -1 for coeff, degree for coeff
  gen primpartcontent(const gen& g,int service,GIAC_CONTEXT){
    vecteur v;
    if (g.type==_VECT && g.subtype !=_SEQ__VECT)
	...
	return r2e(p/f.den,l,contextptr);
      ...
      if (service==-3)
	return r2e(_lgcd(coeffs),ll,contextptr);
      setsizeerr();
    }
    if (s!=1 && s!=2)
      setsizeerr();
    ...
    if (s==2)
      x=v[1];
    gen f(_e2r(makevecteur(v[0],x),contextptr));
    gen deno(1);
    if (f.type==_FRAC){
      deno=f._FRACptr->den;
      f=f._FRACptr->num;
    ...
    case -1:
      return f/deno;
    case -2:
      return symb_horner(*f._VECTptr/_lgcd(f),x);
    case -3:
      return _lgcd(f)/deno;
    }
  ...
  const string _primpart_s("primpart");
  unary_function_eval __primpart(&_primpart,_primpart_s);
  unary_function_ptr at_primpart (&__primpart,0,true);

  gen _content(const gen & g,GIAC_CONTEXT){
    return primpartcontent(g,-3,contextptr);
  }
  const string _content_s("content");
  unary_function_eval __content(&_content,_content_s);
  unary_function_ptr at_content (&__content,0,true);

  const string _icontent_s("icontent");
  unary_function_eval __icontent(&_content,_icontent_s);
  unary_function_ptr at_icontent (&__icontent,0,true);
  ...
  const string _coeff_s("coeff");
  unary_function_eval __coeff(&_coeff,_coeff_s);
  unary_function_ptr at_coeff (&__coeff,0,true);

  const string _coeffs_s("coeffs");
  unary_function_eval __coeffs(&_coeff,_coeffs_s);
  unary_function_ptr at_coeffs (&__coeffs,0,true);

  const string _ichrem_s("ichrem");
  unary_function_unary __ichrem(&_ichinrem,_ichrem_s);
  unary_function_ptr at_ichrem (&__ichrem,0,true);

  gen _chrem(const gen & g){
    ...
    gen res=*it;
    for (++it;it!=itend;++it){
      res=_ichinrem(makevecteur(res,*it));
    }
    return res;
  }
  const string _chrem_s("chrem");
  unary_function_unary __chrem(&_chrem,_chrem_s);
  unary_function_ptr at_chrem (&__chrem,0,true);
  ...
  const string _genpoly_s("genpoly");
  unary_function_eval __genpoly(&_genpoly,_genpoly_s);
  unary_function_ptr at_genpoly (&__genpoly,0,true);

  gen freq_quantile(const matrice & v,double d,GIAC_CONTEXT){
    if (!ckmatrix(v))
      return undef;
    matrice w;
    if (v.size()==2)
  ...
  gen _median(const gen & g,GIAC_CONTEXT){
    ...
    if (!ckmatrix(v))
      return mtran(ascsort(mtran(vecteur(1,v)),true))[(v.size()-1)/2][0];
    else
      v=ascsort(v,true);
    v=mtran(v);
    return v[(v.size()-1)/2];
  }
  const string _median_s("median");
  unary_function_eval unary_median(&_median,_median_s);
  unary_function_ptr at_median (&unary_median,0,true);
  ...
  gen _quartile1(const gen & g,GIAC_CONTEXT){
    ...
    if (!ckmatrix(v))
      return mtran(ascsort(mtran(vecteur(1,v)),true))[(v.size()-1)/4][0];
    else
      v=ascsort(v,true);
    v=mtran(v);
    return v[(v.size()-1)/4];
  }
  const string _quartile1_s("quartile1");
  unary_function_eval unary_quartile1(&_quartile1,_quartile1_s);
  unary_function_ptr at_quartile1 (&unary_quartile1,0,true);
  ...
  gen _quartile3(const gen & g,GIAC_CONTEXT){
    ...
    if (!ckmatrix(v))
      return mtran(ascsort(mtran(vecteur(1,v)),true))[(3*(v.size()-1))/4][0];
    else
      v=ascsort(v,true);
    v=mtran(v);
    return v[(3*(v.size()-1))/4];
  }
  const string _quartile3_s("quartile3");
  unary_function_eval unary_quartile3(&_quartile3,_quartile3_s);
  unary_function_ptr at_quartile3 (&unary_quartile3,0,true);
  ...
  gen _quantile(const gen & g,GIAC_CONTEXT){
    ...
    if (!ckmatrix(v))
      ...
      v=ascsort(v,true);
    v=mtran(v);
    return v[int(d*(v.size()-1))];
  }
  const string _quantile_s("quantile");
  unary_function_eval unary_quantile(&_quantile,_quantile_s);
  unary_function_ptr at_quantile (&unary_quantile,0,true);
  ...
  gen _quartiles(const gen & g,GIAC_CONTEXT){
    ...
    if (s<0)
      setsizeerr();
    return makevecteur(v[0],v[s/4],v[s/2],v[(3*s)/4],v[s]);
  }
  const string _quartiles_s("quartiles");
  unary_function_eval unary_quartiles(&_quartiles,_quartiles_s);
  unary_function_ptr at_quartiles (&unary_quartiles,0,true);
  ...
  gen _moustache(const gen & g_orig,GIAC_CONTEXT){
    ...
    vecteur v0(*_quartiles(g,contextptr)._VECTptr),v;
    if (!ckmatrix(v0))
      v=vecteur(1,v0);
    else
      v=mtran(v0);
	...
	res.push_back(symb_segment(current[3]+y_middle*cst_i,current[4]+y_middle*cst_i,attr,_GROUP__VECT,contextptr));
      ...
      std::swap<double>(xmin,ymin);
      std::swap<double>(xmax,ymax);
  ...
  const string _moustache_s("moustache");
  unary_function_eval unary_moustache(&_moustache,_moustache_s);
  unary_function_ptr at_moustache (&unary_moustache,0,true);

  const string _boxwhisker_s("boxwhisker");
  unary_function_eval unary_boxwhisker(&_moustache,_boxwhisker_s);
  unary_function_ptr at_boxwhisker (&unary_boxwhisker,0,true);


  gen stddevmean(const vecteur & v,int withstddev,int xcol,int freqcol,GIAC_CONTEXT){
    int sv=v.size();
  ...
  gen stddevmean(const gen & g,int withstddev,GIAC_CONTEXT){
    vecteur & v=*g._VECTptr;
    int s=v.size();
    if (s<2)
      setsizeerr();
  ...
  const string _mean_s("mean");
  unary_function_eval __mean(&_mean,_mean_s);
  unary_function_ptr at_mean (&__mean,0,true);

  const string _moyenne_s("moyenne");
  unary_function_eval __moyenne(&_mean,_moyenne_s);
  unary_function_ptr at_moyenne (&__moyenne,0,true);
  ...
  const string _stdDev_s("stdDev");
  unary_function_eval __stdDev(&_stdDev,_stdDev_s);
  unary_function_ptr at_stdDev (&__stdDev,0,true);

  const string _stddevp_s("stddevp");
  unary_function_eval __stddevp(&_stdDev,_stddevp_s);
  unary_function_ptr at_stddevp (&__stddevp,0,true);

  const string _ecart_type_population_s("ecart_type_population");
  unary_function_eval __ecart_type_population(&_stdDev,_ecart_type_population_s);
  unary_function_ptr at_ecart_type_population (&__ecart_type_population,0,true);
  ...
  const string _ecart_type_s("ecart_type");
  unary_function_eval __ecart_type(&_stddev,_ecart_type_s);
  unary_function_ptr at_ecart_type (&__ecart_type,0,true);
  ...
  const string _variance_s("variance");
  unary_function_eval __variance(&_variance,_variance_s);
  unary_function_ptr at_variance (&__variance,0,true);

  vecteur covariance_correlation(const gen & g,const gen & u1,const gen & u2,int xcol,int ycol,int freqcol,GIAC_CONTEXT){
    vecteur v(gen2vecteur(g));
  ...
  void find_xyfreq(const gen & g,gen & gv,int & xcol,int & ycol,int &freqcol){
    xcol=0;
    ycol=1;
    freqcol=-1;
  ...
  gen _covariance_correlation(const gen & g,GIAC_CONTEXT){
    ...
    gen gv;
    find_xyfreq(g,gv,xcol,ycol,freqcol);
    return covariance_correlation(gv,zero,zero,xcol,ycol,freqcol,contextptr);      
  }
  const string _covariance_correlation_s("covariance_correlation");
  unary_function_eval __covariance_correlation(&_covariance_correlation,_covariance_correlation_s);
  unary_function_ptr at_covariance_correlation (&__covariance_correlation,0,true);
  ...
  const string _covariance_s("covariance");
  unary_function_eval __covariance(&_covariance,_covariance_s);
  unary_function_ptr at_covariance (&__covariance,0,true);
  ...
  const string _correlation_s("correlation");
  unary_function_eval __correlation(&_correlation,_correlation_s);
  unary_function_ptr at_correlation (&__correlation,0,true);

  gen _interval2center(const gen & g){
    if (g.type==_VECT)
      return apply(g,_interval2center);
  ...
  const string _interval2center_s("interval2center");
  unary_function_unary __interval2center(&_interval2center,_interval2center_s);
  unary_function_ptr at_interval2center (&__interval2center,0,true);

  void function_regression(const gen & g,const gen & u1,const gen & u2,gen & a,gen &b,double & xmin,double & xmax,gen & correl2,GIAC_CONTEXT){
    gen gv,freq;
    int xcol,ycol,freqcol;
    xmin=1e300;
    xmax=-xmin;
	...
	for (int j=1;j<c;++j){
	  if (u2.type==_FUNC)
	    ...
	    currenty=vy[j];
	  currenty=_interval2center(currenty);
	  freq=w[j];
	  n=n+freq;
	  sigmax=sigmax+currentx*freq;
	  sigmax2=sigmax2+currentx*currentx*freq;
	...
	if (u1.type==_FUNC)
	  ...
	  tmpx=w[xcol];
	tmpx=_interval2center(tmpx);
	if (u2.type==_FUNC)
	  tmpy=u2(w[ycol],contextptr);
	else
	  tmpy=w[ycol];
	tmpy=_interval2center(tmpy);
	if (freqcol<0)
	  freq=plus_one;
	else
	  freq=w[freqcol];
  ...
  gen function_regression(const gen & g,const gen & u1,const gen & u2,GIAC_CONTEXT){
    gen a,b,correl2;
    double xmin,xmax;
    function_regression(g,u1,u2,a,b,xmin,xmax,correl2,contextptr);
  ...
  const string _linear_regression_s("linear_regression");
  unary_function_eval __linear_regression(&_linear_regression,_linear_regression_s);
  unary_function_ptr at_linear_regression (&__linear_regression,0,true);


  gen _exponential_regression(const gen & g,GIAC_CONTEXT){
    return exp(function_regression(g,zero,at_ln,contextptr),contextptr);
  }
  const string _exponential_regression_s("exponential_regression");
  unary_function_eval __exponential_regression(&_exponential_regression,_exponential_regression_s);
  unary_function_ptr at_exponential_regression (&__exponential_regression,0,true);

  gen _logarithmic_regression(const gen & g,GIAC_CONTEXT){
    return function_regression(g,at_ln,zero,contextptr);
  }
  const string _logarithmic_regression_s("logarithmic_regression");
  unary_function_eval __logarithmic_regression(&_logarithmic_regression,_logarithmic_regression_s);
  unary_function_ptr at_logarithmic_regression (&__logarithmic_regression,0,true);
  ...
  const string _power_regression_s("power_regression");
  unary_function_eval __power_regression(&_power_regression,_power_regression_s);
  unary_function_ptr at_power_regression (&__power_regression,0,true);
  ...
  gen _linear_regression_plot(const gen & g,GIAC_CONTEXT){
    ...
    function_regression(G,zero,zero,a,b,xmin,xmax,correl2,contextptr);
    xmax += (xmax-xmin); 
  ...
  const string _linear_regression_plot_s("linear_regression_plot");
  unary_function_eval __linear_regression_plot(&_linear_regression_plot,_linear_regression_plot_s);
  unary_function_ptr at_linear_regression_plot (&__linear_regression_plot,0,true);
  ...
  gen _exponential_regression_plot(const gen & g,GIAC_CONTEXT){
    ...
    bool eq,r;
    gen G=regression_plot_attributs(g,attributs,eq,r,contextptr);
    function_regression(G,zero,at_ln,a,b,xmin,xmax,correl2,contextptr);
    gen ad(evalf_double(a,1,contextptr)),bd(evalf_double(b,1,contextptr)),cd(evalf_double(correl2,1,contextptr));
    if (ad.type==_DOUBLE_ && bd.type==_DOUBLE_ && cd.type==_DOUBLE_){
  ...
  const string _exponential_regression_plot_s("exponential_regression_plot");
  unary_function_eval __exponential_regression_plot(&_exponential_regression_plot,_exponential_regression_plot_s);
  unary_function_ptr at_exponential_regression_plot (&__exponential_regression_plot,0,true);
  ...
  gen _logarithmic_regression_plot(const gen & g,GIAC_CONTEXT){
    ...
    bool eq,r;
    gen G=regression_plot_attributs(g,attributs,eq,r,contextptr);
    function_regression(G,at_ln,zero,a,b,xmin,xmax,correl2,contextptr);
    xmax += (xmax-xmin); 
    gen ad(evalf_double(a,1,contextptr)),bd(evalf_double(b,1,contextptr)),cd(evalf_double(correl2,1,contextptr));
    if (ad.type==_DOUBLE_ && bd.type==_DOUBLE_ && cd.type==_DOUBLE_){
  ...
  const string _logarithmic_regression_plot_s("logarithmic_regression_plot");
  unary_function_eval __logarithmic_regression_plot(&_logarithmic_regression_plot,_logarithmic_regression_plot_s);
  unary_function_ptr at_logarithmic_regression_plot (&__logarithmic_regression_plot,0,true);
  ...
  gen _power_regression_plot(const gen & g,GIAC_CONTEXT){
    ...
    bool eq,r;
    gen G=regression_plot_attributs(g,attributs,eq,r,contextptr);
    function_regression(G,at_ln,at_ln,a,b,xmin,xmax,correl2,contextptr);
    xmax += (xmax-xmin); 
    gen ad(evalf_double(a,1,contextptr)),bd(evalf_double(b,1,contextptr)),cd(evalf_double(correl2,1,contextptr));
    if (ad.type==_DOUBLE_ && bd.type==_DOUBLE_ && cd.type==_DOUBLE_){
  ...
  const string _power_regression_plot_s("power_regression_plot");
  unary_function_eval __power_regression_plot(&_power_regression_plot,_power_regression_plot_s);
  unary_function_ptr at_power_regression_plot (&__power_regression_plot,0,true);

  gen polynomial_regression(const gen & g,int d,const gen & u1, const gen & u2,double & xmin, double & xmax,GIAC_CONTEXT){
  ...
  gen polynomial_regression(const gen & g,double & xmin,double & xmax,GIAC_CONTEXT){
    if (g.type==_VECT && g._VECTptr->size()==3){
      vecteur & v=*g._VECTptr;
  ...
  const string _polynomial_regression_s("polynomial_regression");
  unary_function_eval __polynomial_regression(&_polynomial_regression,_polynomial_regression_s);
  unary_function_ptr at_polynomial_regression (&__polynomial_regression,0,true);
  ...
  const string _polynomial_regression_plot_s("polynomial_regression_plot");
  unary_function_eval __polynomial_regression_plot(&_polynomial_regression_plot,_polynomial_regression_plot_s);
  unary_function_ptr at_polynomial_regression_plot (&__polynomial_regression_plot,0,true);
  ...
  gen logistic_regression(const gen & g,double & xmin,double & xmax,gen & r,GIAC_CONTEXT){
    if (g.type!=_VECT)
      setsizeerr();
    vecteur & v = *g._VECTptr;
    int s=v.size();
    ...
    if (s==2){
      if (n<20)
	setdimerr("Guessing initial production requires more than 20 samples");
  ...
  const string _logistic_regression_s("logistic_regression");
  unary_function_eval __logistic_regression(&_logistic_regression,_logistic_regression_s);
  unary_function_ptr at_logistic_regression (&__logistic_regression,0,true);
  ...
  const string _logistic_regression_plot_s("logistic_regression_plot");
  unary_function_eval __logistic_regression_plot(&_logistic_regression_plot,_logistic_regression_plot_s);
  unary_function_ptr at_logistic_regression_plot (&__logistic_regression_plot,0,true);

  gen gen_interpolate(const gen & g,int deg,GIAC_CONTEXT){
    ...
    if (xmin<x0 || xmax>x1)
      setsizeerr("Values out of range");
    matrice res;
    int pos=0; 
  ...
  const string _linear_interpolate_s("linear_interpolate");
  unary_function_eval __linear_interpolate(&_linear_interpolate,_linear_interpolate_s);
  unary_function_ptr at_linear_interpolate (&__linear_interpolate,0,true);

  gen _parabolic_interpolate(const gen & g,GIAC_CONTEXT){
    return gen_interpolate(g,2,contextptr);
  }
  const string _parabolic_interpolate_s("parabolic_interpolate");
  unary_function_eval __parabolic_interpolate(&_parabolic_interpolate,_parabolic_interpolate_s);
  unary_function_ptr at_parabolic_interpolate (&__parabolic_interpolate,0,true);

  vector<double> prepare_effectifs(const vecteur & v,GIAC_CONTEXT){
    if (v.empty())
      setdimerr();
    vecteur w;
    if (ckmatrix(v)){
  ...
  matrice effectifs(const vecteur & data,const vecteur & intervalles,GIAC_CONTEXT){
  ...
  vecteur centres2intervalles(const vecteur & centres,double class_min,bool with_class_min,GIAC_CONTEXT){
    if (centres.size()<2)
      setsizeerr();
  ...
  const string _center2interval_s("center2interval");
  unary_function_eval __center2interval(&_center2interval,_center2interval_s);
  unary_function_ptr at_center2interval (&__center2interval,0,true);


  gen histogram(const vecteur & v,double class_minimum,double class_size,const vecteur & attributs,GIAC_CONTEXT){
#ifndef WIN32
  ...
  const string _histogram_s("histogram");
  unary_function_eval __histogram(&_histogram,_histogram_s);
  unary_function_ptr at_histogram (&__histogram,0,true);
  ...
  const string _cumulated_frequencies_s("cumulated_frequencies");
  unary_function_eval __cumulated_frequencies(&_cumulated_frequencies,_cumulated_frequencies_s);
  unary_function_ptr at_cumulated_frequencies (&__cumulated_frequencies,0,true);
  ...
  gen _classes(const gen & g,GIAC_CONTEXT){
	...
	if (args[2].type==_VECT && arg1.type==_DOUBLE_){
	  return effectifs(data,centres2intervalles(*args[2]._VECTptr,0.0,false,contextptr),contextptr);
	}
	if (arg1.type==_DOUBLE_ && arg2.type==_DOUBLE_)
  ...
  const string _classes_s("classes");
  unary_function_eval __classes(&_classes,_classes_s);
  unary_function_ptr at_classes (&__classes,0,true);

  vecteur listplot(const gen & g,vecteur & attributs,GIAC_CONTEXT){
    if (g.type!=_VECT || g._VECTptr->empty())
      setsizeerr();
  ...
  const string _listplot_s("listplot");
  unary_function_eval __listplot(&_listplot,_listplot_s);
  unary_function_ptr at_listplot (&__listplot,0,true);
  const string _plotlist_s("plotlist");
  unary_function_eval __plotlist(&_listplot,_plotlist_s);
  unary_function_ptr at_plotlist (&__plotlist,0,true);

  // [[x1 y1] [x2 y2] ...]
  gen scatterplot(const gen & g,bool polygone,bool scatter,GIAC_CONTEXT){
    vecteur v(gen2vecteur(g));
  ...
  const string _scatterplot_s("scatterplot");
  unary_function_eval __scatterplot(&_scatterplot,_scatterplot_s);
  unary_function_ptr at_scatterplot (&__scatterplot,0,true);

  const string _nuage_points_s("nuage_points");
  unary_function_eval __nuage_points(&_scatterplot,_nuage_points_s);
  unary_function_ptr at_nuage_points (&__nuage_points,0,true);

  gen _polygonplot(const gen & g,GIAC_CONTEXT){
    return scatterplot(g,true,false,contextptr);
  }
  const string _polygonplot_s("polygonplot");
  unary_function_eval __polygonplot(&_polygonplot,_polygonplot_s);
  unary_function_ptr at_polygonplot (&__polygonplot,0,true);

  const string _ligne_polygonale_s("ligne_polygonale");
  unary_function_eval __ligne_polygonale(&_polygonplot,_ligne_polygonale_s);
  unary_function_ptr at_ligne_polygonale (&__ligne_polygonale,0,true);

  gen _polygonscatterplot(const gen & g,GIAC_CONTEXT){
    return scatterplot(g,true,true,contextptr);
  }
  const string _polygonscatterplot_s("polygonscatterplot");
  unary_function_eval __polygonscatterplot(&_polygonscatterplot,_polygonscatterplot_s);
  unary_function_ptr at_polygonscatterplot (&__polygonscatterplot,0,true);

  const string _ligne_polygonale_pointee_s("ligne_polygonale_pointee");
  unary_function_eval __ligne_polygonale_pointee(&_polygonscatterplot,_ligne_polygonale_pointee_s);
  unary_function_ptr at_ligne_polygonale_pointee (&__ligne_polygonale_pointee,0,true);

  void read_camembert_args(const gen & g,vecteur & vals,vecteur & names,vecteur & attributs,GIAC_CONTEXT){
    if (g.type!=_VECT)
      setsizeerr();
  ...
  gen _diagramme_batons(const gen & g,GIAC_CONTEXT){
    ...
    read_camembert_args(g,vals,names,attributs,contextptr);
  ...
  const string _diagramme_batons_s("bar_plot");
  unary_function_eval __diagramme_batons(&_diagramme_batons,_diagramme_batons_s);
  unary_function_ptr at_diagramme_batons (&__diagramme_batons,0,true);
  ...
  gen _camembert(const gen & g,GIAC_CONTEXT){
	...
	if (ck_is_strictly_positive(-Vals[i],contextptr))
	  setsizeerr("Negative value encoutered");
  ...
  const string _camembert_s("camembert");
  unary_function_eval __camembert(&_camembert,_camembert_s);
  unary_function_ptr at_camembert (&__camembert,0,true);

  // Graham scan convex hull
 bool graham_sort_function(const gen & a,const gen & b){
  ...
  const string _convexhull_s("convexhull");
  unary_function_eval __convexhull(&_convexhull,_convexhull_s);
  unary_function_ptr at_convexhull (&__convexhull,0,true);
  ...
  const string _simplex_reduce_s("simplex_reduce");
  unary_function_eval __simplex_reduce(&_simplex_reduce,_simplex_reduce_s);
  unary_function_ptr at_simplex_reduce (&__simplex_reduce,0,true);
  ...
  const string _spline_s("spline");
  unary_function_eval __spline(&_spline,_spline_s);
  unary_function_ptr at_spline (&__spline,0,true);

  extern unary_function_ptr at_bitand;
  extern unary_function_ptr at_bitor;
  extern unary_function_ptr at_bitxor;
  extern unary_function_ptr at_hamdist;
  gen giac_bitand(const gen & a,const gen & b){
    ...
    case _ZINT__ZINT:
      ...
      mpz_init(*e);
      mpz_and(*e,*a._ZINTptr,*b._ZINTptr);
      return(e);
    case _INT___ZINT: 
      e =(mpz_t *) malloc(sizeof(mpz_t)); //new mpz_t[1];
      mpz_init_set_ui(*e,a.val);
      mpz_and(*e,*e,*b._ZINTptr);
      return(e);
    case _ZINT__INT_:
      e =(mpz_t *) malloc(sizeof(mpz_t)); //new mpz_t[1];
      mpz_init_set_ui(*e,b.val);
      mpz_and(*e,*a._ZINTptr,*e);
      return(e);
    }
  ...
  gen giac_bitor(const gen & a,const gen & b){
    ...
    case _ZINT__ZINT:
      e =(mpz_t *) malloc(sizeof(mpz_t)); //new mpz_t[1];
      mpz_init(*e);
      mpz_ior(*e,*a._ZINTptr,*b._ZINTptr);
      return(e);
    case _INT___ZINT: 
      e =(mpz_t *) malloc(sizeof(mpz_t)); //new mpz_t[1];
      mpz_init_set_ui(*e,a.val);
      mpz_ior(*e,*e,*b._ZINTptr);
      return(e);
    case _ZINT__INT_:
      e =(mpz_t *) malloc(sizeof(mpz_t)); //new mpz_t[1];
      mpz_init_set_ui(*e,b.val);
      mpz_ior(*e,*a._ZINTptr,*e);
      return(e);
    }
  ...
  gen giac_bitxor(const gen & a,const gen & b){
    ...
    case _ZINT__ZINT:
      e =(mpz_t *) malloc(sizeof(mpz_t)); //new mpz_t[1];
      mpz_init(*e);
      mpz_xor(*e,*a._ZINTptr,*b._ZINTptr);
      return(e);
    case _INT___ZINT: 
      e =(mpz_t *) malloc(sizeof(mpz_t)); //new mpz_t[1];
      mpz_init_set_ui(*e,a.val);
      mpz_xor(*e,*e,*b._ZINTptr);
      return(e);
    case _ZINT__INT_:
      e =(mpz_t *) malloc(sizeof(mpz_t)); //new mpz_t[1];
      mpz_init_set_ui(*e,b.val);
      mpz_xor(*e,*a._ZINTptr,*e);
      return(e);
    }
  ...
  gen giac_hamdist(const gen & a,const gen & b){
    ...
    case _INT___ZINT: 
      mpz_init_set_ui(*e,a.val);
      t=mpz_hamdist(*e,*b._ZINTptr);
      break;
    case _ZINT__INT_:
      mpz_init_set_ui(*e,b.val);
      t=mpz_hamdist(*a._ZINTptr,*e);
      break;
    default:
      delete e;
  ...
  const string _bitand_s("bitand");
  unary_function_eval __bitand(&_bitand,_bitand_s);
  unary_function_ptr at_bitand (&__bitand,0,true);

  gen _bitor(const gen & g,GIAC_CONTEXT){
    return binop(g,giac_bitor);
  }
  const string _bitor_s("bitor");
  unary_function_eval __bitor(&_bitor,_bitor_s);
  unary_function_ptr at_bitor (&__bitor,0,true);

  gen _bitxor(const gen & g,GIAC_CONTEXT){
    return binop(g,giac_bitxor);
  }
  const string _bitxor_s("bitxor");
  unary_function_eval __bitxor(&_bitxor,_bitxor_s);
  unary_function_ptr at_bitxor (&__bitxor,0,true);

  gen _hamdist(const gen & g,GIAC_CONTEXT){
    return binop(g,giac_hamdist);
  }
  const string _hamdist_s("hamdist");
  unary_function_eval __hamdist(&_hamdist,_hamdist_s);
  unary_function_ptr at_hamdist (&__hamdist,0,true);
  ...
  gen _plotarea(const gen & g,GIAC_CONTEXT){
	...
	setsizeerr("plotarea(f(x),x=a..b[,n,method])");
      if (s>=2){
  ...
  const string _plotarea_s("plotarea");
  unary_function_eval __plotarea(&_plotarea,_plotarea_s);
  unary_function_ptr at_plotarea (&__plotarea,0,true);

  const string _areaplot_s("areaplot");
  unary_function_eval __areaplot(&_plotarea,_areaplot_s);
  unary_function_ptr at_areaplot (&__areaplot,0,true);
  ...
  const string _add_language_s("add_language");
  unary_function_eval __add_language(&_add_language,_add_language_s);
  unary_function_ptr at_add_language (&__add_language,0,true);
  ...
  const string _remove_language_s("remove_language");
  unary_function_eval __remove_language(&_remove_language,_remove_language_s);
  unary_function_ptr at_remove_language (&__remove_language,0,true);

  gen _show_language(const gen & args,GIAC_CONTEXT){
    return vector_int_2_vecteur(lexer_localization_vector,contextptr);
  }
  const string _show_language_s("show_language");
  unary_function_eval __show_language(&_show_language,_show_language_s);
  unary_function_ptr at_show_language (&__show_language,0,true);
  ...
  const string _os_version_s("os_version");
  unary_function_eval __os_version(&_os_version,_os_version_s);
  unary_function_ptr at_os_version (&__os_version,0,true);
